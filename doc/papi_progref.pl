...
# papi_progref - generic Perl man page to HTML converter
# customized specifically to generate a PAPI Programmer's Reference,
# and derived from:

# manServer_papi - generic Perl man page to HTML converter
# customized specifically for PAPI documentation, which is
# derived from:
...
sub outputPageFooter
{
	if ($pageName)
	{
		unless ($cmdLineMode)
		{
			...
			plainOutput( "Go to page &nbsp;<INPUT name=q size=12>&nbsp; or view <A HREF=#top>Table of contents for $pageName</A>&nbsp;|&nbsp;\n" );
			plainOutput( "<A HREF=\"$root$dir/\">Section $sectionNumber</A>&nbsp;|&nbsp;\n" );
			plainOutput( "<A HREF=\"$root/\">Main Index</A>\n" );
			plainOutput( "<FORM>\n" );
		}
		endBlockquote();
		outputLine("--PAGE BREAK HERE--\n");
...
sub outputContents
{
	...
	for ($id=1; $id<=$#contents; ++$id)
	{
		...
		outputLine("$pre<A HREF=#$refname$id>$name</A><BR>\n");
	}
	endBlockquote();
}
...
sub processMacro
{
	...
	elsif ($c eq "TH" || $c eq "Dt")			# Man page title
	{
		...
		$sectionName = $Title unless ($sectionName);
	...
	elsif (m/^\.[BI]R (\S+)\s?\(\s?([0-9lL][0-9a-zA-Z]*)\s?\)(.*)$/)
	{
		...
		for ($i=1; $i<=$#p; $i+=2)
		{
			...
			if ($pair =~ m/^(\S+)\s?\(\s?([0-9lL][0-9a-zA-Z]*)\s?\)(.*)$/)
			{
			    ...
			    $lowname = lc($name);
			    $id = "0";
			    outputLine( "<A HREF=#$lowname$id>$name</A>$extra\n" );
...
sub fmtTime
{
    ...
    return sprintf ("%s, %02d %s %4d %02d:%02d:%02d",
...
##
sub usage {
    print LOG <<EndOfUsage;
Usage: $perl papi_progref [ options ] infile
Options:
  -dn              : Debug at level n; 1 and 2 currently supported
  -o <outdir>      : Directory path for HTML output files
  -help            : This message

Description:
  manServer_papi.pl converts troff input formatted manpages into HTML.
  The output file is named by appending ".html" to the end of the input
  file name. An optional output directory can be specified.
