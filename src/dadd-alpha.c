      ...
      if (papi_code != PAPI_NULL) {
         ...
         if (ptr_vc) {
            memcpy(&count,
                   ...
                   sizeof(unsigned long) * dadd_code, sizeof(unsigned long));
...
*/
         ...
         if ((ESI->EventInfoArray[i].code == PAPI_FLOPS) ||
            ...
            if (cycles)
               events[i] = events[i] * _papi_system_info.hw_info.mhz * 1000000 / cycles;
            else
               events[i] = 0;
         }
      }
...
int _papi_hwd_set_profile(EventSetInfo_t * ESI, EventSetProfileInfo_t * overflow_opt ion)
{
   /* This function is not used and shouldn't be called. */
