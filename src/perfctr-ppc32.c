...
int
setup_ppc32_presets( int cputype )
{
	int i = 0;
	switch ( cputype ) {
	...
	while ( native_table[i].resources.selector )
		i++;
...
_papi_hwd_set_domain( hwd_control_state_t * cntrl, int domain )
{
	...
	if ( ( domain == PAPI_DOM_ALL ) ||
		 ( domain == ( PAPI_DOM_USER | PAPI_DOM_KERNEL ) ) ) {
		did = 1;
...
_papi_hwd_bpt_map_preempt( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	...
	for ( i = 0, dst->ra_rank = 0; i < _papi_hwi_system_info.sub_info.num_cntrs;
		  i++ )
...
_papi_hwd_allocate_registers( EventSetInfo_t * ESI )
{
	...
	for ( i = 0; i < natNum; i++ ) {
		...
		memcpy( &event_list[i].ra_bits, &native_table[index].resources,
				sizeof ( event_list[i].ra_bits ) );
	...
	if ( _papi_hwi_bipartite_alloc( event_list, natNum ) ) {
		...
		if ( contr->cpu_control.ppc.mmcr0 & PERF_INT_PMCxEN ) {
			...
			for ( i = 0; i < natNum; i++ ) {
				if ( contr->cpu_control.pmc_map[i] > 0 ) {
					if ( ++pmcx_ov_check > 1 ) {
						PAPIERROR
							( "Only 1 event on PMC2-n allowed if overflow is enabled!" );
						return ( 0 );
					}
				}
			}
		...
		for ( i = 0; i < natNum; i++ ) {
			...
			this_state->control.cpu_control.pmc_map[i] =
				ffs( event_list[i].ra_selector ) - 1;
			SUBDBG
				( "bipartite put event_list[%d].ra_selector is %08X, %08X on PMC %d\n",
				  i, event_list[i].ra_selector, event_list[i].ra_bits.selector,
				  ffs( event_list[i].ra_selector ) - 1 );
		}
		return 1;
	} else
...
static void
clear_control_state( hwd_control_state_t * this_state )
{
								...
								hwd_context_t * ctx )
{
	...
	for ( i = 0; i < count; i++ ) {
		SUBDBG( "event %d, counter_cmd 0x%x, selector 0x%x\n", i,
				native[i].ni_bits.counter_cmd, native[i].ni_bits.selector );
...
int
ppc32_setup_vector_table( papi_vectors_t * vtable )
{
	int retval = PAPI_OK;
