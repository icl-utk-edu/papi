...
static hwd_preset_t p6_preset_map[PAPI_MAX_PRESET_EVENTS] = { 
  {ALLCNTRS,0,0,{{0x45,0x45,0x0,0x0}},""},	// L1 Cache Dmisses 
  {ALLCNTRS,0,0,{{0x81,0x81,0x0,0x0}},""},	// L1 Cache Imisses 
  ...
  {ALLCNTRS,0,0,{{0xf2e,0xf2e,0x0,0x0}},""},	// L1 Total Cache misses 
  {ALLCNTRS,0,0,{{0x24,0x24,0x0,0x0}},""}, 	// L2 Total Cache misses
  {0,0,0,{{0,0,0x0,0x0}},""}, 			// L3 Total Cache misses
  {0,0,0,{{0,0,0x0,0x0}},""},			// Snoops
  {ALLCNTRS,0,0,{{0x222e,0x222e,0x0,0x0}},""},	// Req. access to shared cache line
  {ALLCNTRS,0,0,{{0x212e,0x212e,0x0,0x0}},""},	// Req. access to clean cache line
  {ALLCNTRS,0,0,{{0x2069,0x2069,0x0,0x0}},""},	// Req. Cache Line Invalidation
  {ALLCNTRS,0,0,{{0x2e2e,0x2e2e,0x0,0x0}},""},	// Req. Cache Line Intervention
  ...
  {ALLCNTRS,0,0,{{0x85,0x85,0x0,0x0}},""},	// I-TLB misses
  {0,0,0,{{0,0,0x0,0x0}},""},			// Total TLB misses
  {ALLCNTRS,0,0,{{0xf29,0xf29,0x0,0x0}},""},	// L1 load M
  {ALLCNTRS,0,0,{{0xf2A,0xf2A,0x0,0x0}},""},	// L1 store M
  {0,0,0,{{0,0,0x0,0x0}},""},			// L2 load M
  {0,0,0,{{0,0,0x0,0x0}},""},			// L2 store M
  {ALLCNTRS,0,0,{{0xe2,0xe2,0x0,0x0}},""},	// BTAC misses
  ...
  {ALLCNTRS,0,0,{{0xC8,0xC8,0x0,0x0}},""},	// hardware interrupts
  {0,0,0,{{0,0,0x0,0x0}},""},	                // Uncond. branches executed
  {ALLCNTRS,0,0,{{0xC4,0xC4,0x0,0x0}},""},	// Cond. Branch inst. executed
  {ALLCNTRS,0,0,{{0xC9,0xC9,0x0,0x0}},""},	// Cond. Branch inst. taken
  {CNTR2|CNTR1,DERIVED_SUB,0,{{0xC4,0xC9,0x0,0x0}},""}, // Cond. Branch inst. not taken
  {ALLCNTRS,0,0,{{0xC5,0xC5,0x0,0x0}},""},	// Cond. branch inst. mispred.
  {CNTR2|CNTR1,DERIVED_SUB,0,{{0xC4,0xC5,0x0,0x0}},""}, // Cond. branch inst. corr. pred.
  {0,0,0,{{0,0,0x0,0x0}},""},			// FMA
  {ALLCNTRS,0,0,{{0xD0,0xD0,0x0,0x0}},""},	// Total inst. issued
  {ALLCNTRS,0,0,{{0xC0,0xC0,0x0,0x0}},""},	// Total inst. executed
  ...
  {ALLCNTRS,0,0,{{0xA2,0xA2,0x0,0x0}},""},	// Cycles any resource stalls
  {0,0,0,{{0,0,0x0,0x0}},""},			// Cycles FPU stalled
  {ALLCNTRS,0,0,{{0x79,0x79,0x0,0x0}},""},	// Total cycles
  ...
  {ALLCNTRS,0,0,{{0x80,0x80,0x0,0x0}},""},	// L1_ICA
  {ALLCNTRS,0,0,{{0xf28,0xf28,0x0,0x0}},""},	// L2_ICA
  {0,0,0,{{0,0,0x0,0x0}},""},			// L3_ICA
  {ALLCNTRS,0,0,{{0x80,0x80,0x0,0x0}},""},	// L1_ICR
  {ALLCNTRS,0,0,{{0xf28,0xf28,0x0,0x0}},""},	// L2_ICR
  {0,0,0,{{0,0,0x0,0x0}},""},			// L3_ICR
  {ALLCNTRS,0,0,{{0x81,0x81,0x0,0x0}},""},	// L1_ICW
  ...
  {ALLCNTRS,0,0,{{0xf2e,0xf2e,0x0,0x0}},""},	// L2_TCA
  ...
  {ALLCNTRS,0,0,{{0xf2a,0xf2a,0x0,0x0}},""},	// L2_TCW
...
#ifdef PERFCTR20PRE4
inline static void set_hwcntr_codes(int selector, struct perfctr_cpu_control_user *from, struct perfctr_cpu_control_user *to)
#else
...
inline static void init_config(hwd_control_state_t *ptr)
{
  ...
  ptr->counter_cmd.cpu_control.nrctrs=2;
...
#ifndef PERFCTR20PRE4 /* Skip truncating MHz value */
  {
...
#ifdef PERFCTR20PRE4
  DBG((stderr,"tsc_on=0x%x  nrctrs=0x%x, nrictrs=0x%x\n",
       t->cpu_control.tsc_on,t->cpu_control.nrctrs,t->cpu_control.nrictrs));
  for (i=0;i<_papi_system_info.num_cntrs;i++)
  ...
  for (i=0;i<_papi_system_info.num_cntrs;i++)
    {
      ...
      DBG((stderr,"update_global_hwcounters() %d: G%lld = G%lld + C%lld\n",i,
	   global->hw_start[i]+state.head.sum.pmc[i],
	   global->hw_start[i],state.head.sum.pmc[i]));
      global->hw_start[i] = global->hw_start[i] + state.head.sum.pmc[i];
#else
      DBG((stderr,"update_global_hwcounters() %d: G%lld = G%lld + C%lld\n",i,
	   global->hw_start[i]+state.head.sum.pmc[i+1],
	   global->hw_start[i],state.head.sum.pmc[i+1]));
      global->hw_start[i] = global->hw_start[i] + state.head.sum.pmc[i+1];
#endif
    }
...
int _papi_hwd_add_event(hwd_control_state_t *this_state, unsigned int EventCode, EventInfo_t *out)
{
  ...
  int avail = 0;
#ifdef PERFCTR20PRE4
  struct perfctr_cpu_control_user tmp_cmd, *codes;
#else
