 ...
 */
inline BGP_UPC_Event_Id_t get_bgp_native_event_id(int pEventId) {
...
int _papi_bgp_get_system_info(void) {
  ...
  _papi_hwi_system_info.hw_info.nnodes = (int) BGP_Personality_numComputeNodes(&bgp);
 ...
 */
inline_static int setup_bgp_presets(int cpu_type) {
  switch ( cpu_type ) {
    default:
...
//  printf("_papi_bgp_read:  (long_long*)&this_state->counters[1] = %p\n", (long_long*)&this_state->counters[1]);
  ...
  sigprocmask(SIG_BLOCK, &mask_set, &old_set);
    ...
    if (BGP_UPC_Read_Counters((long_long*)&this_state->counters[0], BGP_UPC_MAXIMUM_LENGTH_READ_COUNTERS_ONLY, BGP_UPC_READ_EXCLUSIVE) < 0) {
    sigprocmask(SIG_UNBLOCK, &mask_set, NULL);
    return PAPI_ESBSTR;
    }
   sigprocmask(SIG_UNBLOCK, &mask_set, NULL);
...
void user_signal_handler(int signum, siginfo_t *siginfo, void *mycontext) {
	...
	if (ESI == NULL) {
		...
		for (i = 0; i <= event_index; i++) {
			xEventId = get_bgp_native_event_id(ESI->NativeInfoArray[i].ni_event);		
			if (BGP_UPC_Read_Counter(xEventId, 1) >= BGP_UPC_Get_Counter_Threshold_Value(xEventId) && BGP_UPC_Get_Counter_Threshold_Value(xEventId) != 0) { 
				break;
			}
		}
...
//  return (long long)(x/y);
  ...
  return ((long long)(((float)get_cycles())/((_papi_hwi_system_info.hw_info.mhz))));
}
