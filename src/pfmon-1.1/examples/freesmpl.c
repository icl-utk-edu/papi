...
int
child(char **arg, pfmlib_param_t *evt, int cnt)
{
	int i;
	pid_t pid = getpid();
...
monitor(char **arg)
{
	 ...
	 */
	smpl_regs = 0;
	for(i=1; i < evt.pfp_count; i++) {
		smpl_regs |= 1UL << pc[i].reg_num;
	}
	pc[0].reg_reset_pmds[0] = smpl_regs;
