...
gen_thresholds(char *arg, pfmlib_param_t *evt)
{
	...
	if (arg == NULL) {
		int i;
		for (i=0; i < evt->pfp_count; i++) param->pfp_ita_counters[i].thres = 0;
		return;
	}
	while (arg) {
 ...
 */
static int
pfmon_ita_install_counters(int pid, pfmlib_param_t *evt, pfarg_reg_t *pc, int count, pfarg_reg_t *pd)
{
	pfmlib_ita_param_t *param = ITA_PARAM(evt);
	...
	if (param->pfp_ita_btb.btb_used) prepare_pmd16(pid, evt, pc);
...
setup_ear(pfmlib_param_t *evt)
{
	...
	for (i=0; i < evt->pfp_count; i++) {
		if (pfm_ita_is_ear(evt->pfp_evt[i]) == 0) continue;

		if (pfm_ita_is_dear(evt->pfp_evt[i])) {
			if (done_dear) {
				fatal_error("cannot specify more than one D-EAR event at the same time\n");
			}
			pfmon_ita_opt.opt_use_dear_tlb = pfm_ita_is_dear_tlb(evt->pfp_evt[i]) ? 1 : 0;
		...
		if (pfm_ita_is_iear(evt->pfp_evt[i])) {
			if (done_iear) {
				fatal_error("cannot specify more than one D-EAR event at the same time\n");
			}
			pfmon_ita_opt.opt_use_iear_tlb = pfm_ita_is_iear_tlb(evt->pfp_evt[i]) ? 1 : 0;
...
found:
	...
	param->pfp_ita_btb.btb_plm = evt->pfp_plm[i]; /* use the plm from the BTB event */
...
static int
gen_ita_insn_sets(char *arg, pfmlib_ita_counter_t *c)
{
	static const struct {
		char *name;
		unsigned int val;
...
pfmon_ita_parse_options(int code, char *optarg, pfmlib_param_t *evt)
{
	...
	switch(code) {
		...
		case  407:
			...
			if (gen_ita_insn_sets(optarg, param->pfp_ita_counters) == -1) fatal_error("");
...
check_ita_event_combinations(pfmlib_param_t *evt)
{
	...
	for (i=0; i < evt->pfp_count; i++) {
		...
		if (use_opcm && pfm_ita_support_opcm(cnt_list[i]) == 0)
			fatal_error("event %s does not support opcode matching\n", name);

		if (param->pfp_ita_irange.rr_used && pfm_ita_support_iarr(cnt_list[i]) == 0)
