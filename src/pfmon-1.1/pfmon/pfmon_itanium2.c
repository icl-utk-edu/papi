...
gen_thresholds(char *arg, pfmlib_param_t *evt)
{
	...
	if (arg == NULL) {
		int i;
		for (i=0; i < evt->pfp_count; i++) param->pfp_ita2_counters[i].thres = 0;
		return;
	}
	while (arg) {

		if (cnt == options.max_counters || cnt == evt->pfp_count) goto too_many;
		...
		pfm_ita2_get_event_maxincr(evt->pfp_evt[cnt], &maxincr);
...
static void
prepare_pmd16(pid_t pid, pfmlib_param_t *evt, pfarg_reg_t *pc)
{
	pfarg_reg_t pdx[1];
	...
	for(i=0; i < evt->pfp_count; i++) {
		if (pfm_ita2_is_btb(cnt_list[i])) {
			/*
			 * add pmd16 to the set of pmds to reset when pd[i].reg_num 
			 * overflows
			 */
			pc[i].reg_reset_pmds[0] |= 1UL << 16;
...
static void
fixup_ears(pid_t pid, pfmlib_param_t *evt, pfarg_reg_t *pc)
{
	int *cnt_list = evt->pfp_evt;
	int i;
	...
	for(i=0; i < evt->pfp_count; i++) {
		if (pfm_ita2_is_ear(cnt_list[i])) {
			 ...
			 */
			if (pfm_ita2_is_dear(cnt_list[i])) 
				pc[i].reg_reset_pmds[0] |= 1UL << 3 | 1UL << 17;
		}
	}
}
 ...
 */
static int
pfmon_ita2_install_counters(int pid, pfmlib_param_t *evt, pfarg_reg_t *pc, int count, pfarg_reg_t *pd)
{
	pfmlib_ita2_param_t *param = ITA2_PARAM(evt);
	...
	if (param->pfp_ita2_btb.btb_used) prepare_pmd16(pid, evt, pc);

	fixup_ears(pid, evt, pc);
...
setup_ear(pfmlib_param_t *evt)
{
	...
	for (i=0; i < evt->pfp_count; i++) {
		if (pfm_ita2_is_ear(evt->pfp_evt[i]) == 0) continue;

		if (pfm_ita2_is_dear(evt->pfp_evt[i])) {
			if (done_dear) {
				fatal_error("cannot specify more than one D-EAR event at the same time\n");
			}

			pfm_ita2_get_ear_mode(evt->pfp_evt[i], &pfmon_ita2_opt.dear_mode);

			param->pfp_ita2_dear.ear_used   = 1;
			param->pfp_ita2_dear.ear_mode   = pfmon_ita2_opt.dear_mode;
			pfm_ita2_get_event_umask(evt->pfp_evt[i], &param->pfp_ita2_dear.ear_umask);
			param->pfp_ita2_dear.ear_plm    = evt->pfp_plm[i]; /* use plm from event */
			param->pfp_ita2_dear.ear_ism    = param->pfp_ita2_counters[i].ism;
			
			options.smpl_regs |=  pfm_ita2_is_dear_alat(evt->pfp_evt[i]) ? DEAR_ALAT_REGS_MASK : DEAR_REGS_MASK;

			done_dear = 1;
		}

		if (pfm_ita2_is_iear(evt->pfp_evt[i])) {
			if (done_iear) {
				fatal_error("cannot specify more than one D-EAR event at the same time\n");
			}
			pfm_ita2_get_ear_mode(evt->pfp_evt[i], &pfmon_ita2_opt.iear_mode);

			param->pfp_ita2_iear.ear_used   = 1;
			param->pfp_ita2_iear.ear_mode   = pfmon_ita2_opt.iear_mode;
			pfm_ita2_get_event_umask(evt->pfp_evt[i], &param->pfp_ita2_iear.ear_umask);
			param->pfp_ita2_iear.ear_plm    = evt->pfp_plm[i]; /* use plm from event */
...
setup_btb(pfmlib_param_t *evt)
{
	 ...
	 */
	for (i=0; i < evt->pfp_count; i++) {
		if (pfm_ita2_is_btb(evt->pfp_evt[i])) found_btb = 1;
		if (pfm_ita2_is_dear_alat(evt->pfp_evt[i])) found_alat = 1;
		if (pfm_ita2_is_dear_tlb(evt->pfp_evt[i])) found_dear = 1;
	}
	if (found_btb == 0) {
		if (  pfmon_ita2_opt.opt_btb_ds
		   || pfmon_ita2_opt.opt_btb_tm
		   || pfmon_ita2_opt.opt_btb_ptm
		   || pfmon_ita2_opt.opt_btb_ppm
		   || pfmon_ita2_opt.opt_btb_brt) goto found;
		return 0;
	}
found:
	/*
	...
	param->pfp_ita2_btb.btb_plm = evt->pfp_plm[i]; /* use the plm from the BTB event */
...
static int
gen_ita2_insn_sets(char *arg, pfmlib_ita2_counter_t *c)
{
	static const struct {
		char *name;
		unsigned int val;
...
error:
	if (p) *p = ',';
	warning("unknown per event instruction set %s (choices are ia32, ia64, both)\n", arg);
	return -1;
too_many:
	warning("too many per event instruction sets specified, max=%d\n", options.max_counters);
	return -1;
}
...
pfmon_ita2_parse_options(int code, char *optarg, pfmlib_param_t *evt)
{
	...
	switch(code) {
		...
		case  406:
			...
			break;
		case  407:
			if (fine_insn_sets_done) fatal_error("per event instruction sets already defined");
			if (gen_ita2_insn_sets(optarg, param->pfp_ita2_counters) == -1) fatal_error("");
			fine_insn_sets_done = 1;
			break;
...
check_cancel_events(pfmlib_param_t *evt)
{
	...
	for(i=0; i < NCANCEL_EVENTS; i++) {
		pfm_find_event(cancel_events[i], 0, &tmp);
		...
		for (j=0; j < NCANCEL_EVENTS; j++) {
			...
			if (code == cancel_codes[j]) {
				if (idx != -1) {
					...
					pfm_get_event_name(evt->pfp_evt[i], &name2);
...
check_cross_groups_and_set_umask(pfmlib_param_t *evt)
{
	...
	for (i=0; i < cnt; i++) {
		...
		pfm_ita2_get_event_group(cnt_list[i], &g);
		pfm_ita2_get_event_set(cnt_list[i], &s);

		if (g == PFMLIB_ITA2_EVT_NO_GRP) continue;

		pfm_ita2_get_event_umask(cnt_list[i], &ref_umask);

		for (j=i+1; j < cnt; j++) {
			pfm_ita2_get_event_group(cnt_list[j], &g2);
			if (g2 != g) continue;

			pfm_ita2_get_event_set(cnt_list[j], &s2);
			if (s2 != s) goto error;

			/* only care about L2 cache group */
			if (g != PFMLIB_ITA2_EVT_L2_CACHE_GRP || (s == 1 || s == 2)) continue;

			pfm_ita2_get_event_umask(cnt_list[j], &umask);
			/*
...
{
	char *name;
	int i, use_opcm, inst_retired_idx = -1;
	int *cnt_list  = evt->pfp_evt;
	pfmlib_ita2_param_t *param = ITA2_PARAM(evt);
	...
	pfm_find_event("IA64_INST_RETIRED", 0, &inst_retired_idx);

	for (i=0; i < evt->pfp_count; i++) {

		pfm_get_event_name(cnt_list[i], &name);

		if (use_opcm && pfm_ita2_support_opcm(cnt_list[i]) == 0)
			fatal_error("event %s does not support opcode matching\n", name);

		if (param->pfp_ita2_pmc9.opcm_used && cnt_list[i] != inst_retired_idx) 
			fatal_error("pmc9 can only be used to qualify the IA64_INST_RETIRED events\n");

		if (param->pfp_ita2_irange.rr_used && pfm_ita2_support_iarr(cnt_list[i]) == 0)
			fatal_error("event %s does not support instruction address range restrictions\n", name);

		if (param->pfp_ita2_drange.rr_used && pfm_ita2_support_darr(cnt_list[i]) == 0)
...
pfmon_ita2_post_options(pfmlib_param_t *evt)
{
	...
	if (pfmon_ita2_opt.opt_ia32  && pfmon_ita2_opt.opt_ia64)
		param->pfp_ita2_ism = PFMLIB_ITA2_ISM_BOTH;
	else if (pfmon_ita2_opt.opt_ia64)
		param->pfp_ita2_ism = PFMLIB_ITA2_ISM_IA64;
	else if (pfmon_ita2_opt.opt_ia32)
		param->pfp_ita2_ism = PFMLIB_ITA2_ISM_IA32;
	else
		param->pfp_ita2_ism = PFMLIB_ITA2_ISM_BOTH;
...
pfmon_ita2_print_header(FILE *fp)
{
	...
	for(i=0; i < PMU_MAX_PMDS; i++) {
		...
		fprintf(fp, "#\tPMD%d: %s, %s\n", 
			i,
			name,
			insn_str[isn ? isn : pfmon_ita2_opt.params->pfp_ita2_ism]);
	} 
	fprintf(fp, "#\n");
...
pfmon_ita2_detailed_event_name(int evt)
{
	...
	if (grp != PFMLIB_ITA2_EVT_NO_GRP) {
		...
		printf("umask=0x%02lx incr=%ld grp=%s set=%d", umask, maxincr, grp_str, set);
	} else {
