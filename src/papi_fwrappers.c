...
#elif defined(_FORTRAN_STRLEN_AT_END)
PAPI_FCALL(papif_get_preload, PAPIF_GET_PRELOAD, (char *lib_preload_env, int *check, int lib_preload_env_len))
#else
...
#endif
{
  ...
  if (!info[index].event_name) *check = PAPI_ENOEVNT;
    ...
    strncpy(event_name, info[index].event_name, event_name_len);
    for(i=strlen(info[index].event_name);i<event_name_len;event_name[i++]=' ');

    if(info[index].event_descr){
      strncpy(event_descr, info[index].event_descr, event_descr_len);
      for(i=strlen(info[index].event_descr);i<event_descr_len;event_descr[i++]=' ');
    }
    else{
      for(i=0;i<event_descr_len;event_descr[i++]=' ');
    }

    if(info[index].event_label){
      strncpy(event_label, info[index].event_label, event_label_len);
      for(i=strlen(info[index].event_label);i<event_label_len;event_label[i++]=' ');
    }
    else{
      for(i=0;i<event_label_len;event_label[i++]=' ');
    }

    *avail = info[index].avail;

    if(info[index].event_note){
      strncpy(event_note, info[index].event_note, event_note_len);
      for(i=strlen(info[index].event_note);i<event_note_len;event_note[i++]=' ');
    }
    else{
      for(i=0;i<event_note_len;event_note[i++]=' ');
    }
