...
int _p4_ntv_enum_events(unsigned int *EventCode, int modifier)
{
...
int _p4_ntv_code_to_bits(unsigned int EventCode, hwd_register_t * bits)
{
   ...
   *(cmp_register_t *)bits = _p4_native_map[event].bits;
   ...
   ((cmp_register_t *)bits)->cccr |= CCCR_THR_MODE(CCCR_THR_ANY) | CCCR_ENABLE;

   return (PAPI_OK);
}

int _p4_ntv_encode(unsigned int *EventCode, char *name, char *description,
                         hwd_register_t * bits)
{
...
int _p4_ntv_decode(unsigned int EventCode, char *name, char *description,
                         hwd_register_t * bits)
{
   char *str;
...
int _p4_ntv_bits_to_info(hwd_register_t *bits, char *names,
...
#define   FP_SP 2
#define   FP_DP 4
void _p4_fixup_fp(hwi_search_t **s, const hwi_dev_notes_t **n)
{
   ...
   if (MY_VECTOR.cmp_info.num_native_events == 0)
     MY_VECTOR.cmp_info.num_native_events = (sizeof(_p4_native_map) + sizeof(_p4_custom_map))/sizeof(hwd_p4_native_map_t);
...
#define  VEC_MMX 1
#define  VEC_SSE 2
void _p4_fixup_vec(hwi_search_t **s, const hwi_dev_notes_t **n)
{
