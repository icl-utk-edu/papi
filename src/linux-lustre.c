 ...
 */
int LUSTRE_init(hwd_context_t *ctx)
{
...
int LUSTRE_shutdown(hwd_context_t *ctx)
{
   host_finalize();
   return(PAPI_OK);
}
 ...
 * functions
 */
int LUSTRE_init_control_state(hwd_control_state_t *ptr){
   return PAPI_OK;
}

int LUSTRE_update_control_state(hwd_control_state_t *ptr, NativeInfo_t *native, int count, hwd_context_t *ctx){
   int i, index;
...
int LUSTRE_start(hwd_context_t *ctx, hwd_control_state_t *ctrl){
   host_read_values(_papi_hwd_lustre_register_start);
   memcpy(_papi_hwd_lustre_register, _papi_hwd_lustre_register_start, LUSTRE_MAX_COUNTERS*sizeof(long long));

   return(PAPI_OK);
}


int LUSTRE_read(hwd_context_t *ctx, hwd_control_state_t *ctrl, long long **events, int flags)
{
    int i;

    host_read_values(_papi_hwd_lustre_register); 
    for(i = 0; i < ((LUSTRE_control_state_t *)ctrl)->ncounter; i++){
       ((LUSTRE_control_state_t *)ctrl)->counts[i] = _papi_hwd_lustre_register[i] - _papi_hwd_lustre_register_start[i];
      /*printf("%d  %lld\n", i, ctrl->counts[i]);*/
    }
...
int LUSTRE_stop(hwd_context_t *ctx, hwd_control_state_t *ctrl)
{
   int i;
...
int LUSTRE_reset(hwd_context_t *ctx, hwd_control_state_t *ctrl)
{
...
int LUSTRE_write(hwd_context_t *ctx, hwd_control_state_t *ctrl, long long *from)
{
   return(PAPI_OK);
}
 ...
 */
int LUSTRE_ctl(hwd_context_t *ctx, int code, _papi_int_option_t *option)
{
  return(PAPI_OK);
}
 ...
 */
int LUSTRE_set_domain(hwd_control_state_t *cntrl, int domain) 
{
  int found = 0;
...
int LUSTRE_ntv_code_to_name(unsigned int EventCode, char *name, int len)
{
   strncpy(name, lustre_native_table[EventCode & PAPI_NATIVE_AND_MASK & PAPI_COMPONENT_AND_MASK]->name, len);
   return(PAPI_OK);
}

int LUSTRE_ntv_code_to_descr(unsigned int EventCode, char *name, int len)
{
   strncpy(name, lustre_native_table[EventCode & PAPI_NATIVE_AND_MASK & PAPI_COMPONENT_AND_MASK]->description, len);
   return(PAPI_OK);
}

int LUSTRE_ntv_code_to_bits(unsigned int EventCode, hwd_register_t * bits)
{
...
int LUSTRE_ntv_bits_to_info(hwd_register_t *bits, char *names, unsigned int *values, int name_len, int count)
{
  return(1);
}


int LUSTRE_ntv_enum_events(unsigned int *EventCode, int modifier)
{
