...
static int sys_vperfctr_stop(struct vperfctr *perfctr, int is_remote)
{
	if( IS_RUNNING(perfctr) ) {
		if( !is_remote ) /* not needed for remote access */
			vperfctr_suspend(perfctr);
		perfctr->state.cpu_state.cstatus = 0;
#if PERFCTR_INTERRUPT_SUPPORT
		perfctr->iresume_cstatus = 0;
				...
				int is_remote)
{
	...
	if( copy_from_user(&control, argp, sizeof control) )
		return -EFAULT;
	sys_vperfctr_stop(perfctr, is_remote);
...
{
	task_thread(tsk ? tsk : current)->perfctr = NULL;
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	case VPERFCTR_CONTROL:
		...
		break;
	case VPERFCTR_STOP:
		ret = sys_vperfctr_stop(perfctr, tsk != NULL);
		break;
