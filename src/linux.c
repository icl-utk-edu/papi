...
_linux_update_shlib_info( void )
{
	...
	tmp =
		( PAPI_address_map_t * ) papi_calloc( ( size_t ) upper_bound,
											  sizeof ( PAPI_address_map_t ) );
	if ( tmp == NULL ) {
		PAPIERROR( "calloc(%d) failed",
				   upper_bound * ( int ) sizeof ( PAPI_address_map_t ) );
		fclose( f );
		return ( PAPI_OK );
	}
	...
	tmp2 =
		( PAPI_address_map_t * ) papi_calloc( ( size_t ) count,
											  sizeof ( PAPI_address_map_t ) );
	if ( tmp2 == NULL ) {
		PAPIERROR( "calloc(%d) failed",
				   count * ( int ) sizeof ( PAPI_address_map_t ) );
		papi_free( tmp );
		fclose( f );
		return ( PAPI_OK );
	}
