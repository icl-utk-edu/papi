...
int _papi_hwd_write( hwd_context_t *, hwd_control_state_t *,
					 long long events[] );
int _papi_hwd_cleanup_eventset( hwd_control_state_t * );
...
void CALLBACK _papi_hwd_timer_callback( UINT wTimerID, UINT msg, DWORD dwUser,
										DWORD dw1, DWORD dw2 );
#else
 ...
 */
int _papi_hwd_ntv_code_to_name( unsigned int EventCode, char *hwd_name,
								int len );
int _papi_hwd_ntv_code_to_descr( unsigned int EventCode, char *hwd_descr,
								 int len );
