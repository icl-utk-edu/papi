...
int
_papi_hwd_get_memory_info (PAPI_hw_info_t * hw, int id)
{
  ...
  mem[0].cache[0].num_lines = mem[0].cache[0].size / mem[0].cache[0].line_size;
  mem[0].cache[0].associativity = 8;

  /* D-Cache -> L1$ data */
  mem[0].cache[1].type = PAPI_MH_TYPE_DATA | PAPI_MH_TYPE_WT | PAPI_MH_TYPE_LRU;
  mem[0].cache[1].size = 8 * 1024;     // 8 Kb
  mem[0].cache[1].line_size = 16;
  mem[0].cache[1].num_lines = mem[0].cache[1].size / mem[0].cache[1].line_size;
  ...
  mem[1].cache[0].num_lines = mem[1].cache[0].size / mem[1].cache[0].line_size;
