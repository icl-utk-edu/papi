 ...
 */
static int
_internal_set_domain( hwd_control_state_t * this_state, int domain )
{
	int found = 0;
 ...
 */
static int
_internal_set_granularity( hwd_control_state_t * this_state, int granularity )
{
	switch ( granularity ) {
	case PAPI_GRN_THR:
		break;
...
{
	return ( _papi_hwd_get_virt_usec( zero ) *
			 ( long long ) _papi_system_info.hw_info.mhz );
}
