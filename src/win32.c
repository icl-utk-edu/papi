...
{
//	volatile ULARGE_INTEGER time;

	__asm rdtsc						// Read Time Stamp Counter
//	__asm mov time.LowPart, eax		// move low part to union
//	__asm mov time.HighPart, edx	// move high part to union
//    return (time.QuadPart);			// send 64 bits back
// moving this value to a stack-based variable and then returning it
// has the effect of moving it from edx:eax and back...
// we should be able to just execute rdtsc and return
}
...
static_inline u_long_long get_cycles (void)
{
	u_long_long ret;
...
inline_static int setup_all_presets(int cpu_type)
{
  int pnum, s;
  char note[100];
...
inline_static void set_hwcntr_codes(int selector, struct perfctr_control *from, struct perfctr_control *to)
{
  int useme, i;
...
static void winRevision(SYSTEM_INFO siSysInfo, char *vendor, float rev)
{
	switch (siSysInfo.wProcessorArchitecture)
	{
	  case PROCESSOR_ARCHITECTURE_INTEL:
		  switch (siSysInfo.wProcessorLevel)
		  {
			...
			case 2:
			case 3:
			  strcpy(vendor, "Intel 80386");
			  break;
			case 4:
			  strcpy(vendor, "Intel 80486");
			  break;
			case 5:
			  strcpy(vendor, "Intel Pentium");
			  break;
			case 6:
			  strcpy(vendor, "Intel Pentium Pro or II");
			  break;
			default:
			  strcpy(vendor, "Intel Pentium III or above");
			  break;
		  }
		  if (siSysInfo.wProcessorLevel < 5) {
			if (HIBYTE(siSysInfo.wProcessorLevel) == 0xFF)
				rev = (float)(siSysInfo.wProcessorLevel & 0x000F);
			else rev = (float)(HIBYTE(siSysInfo.wProcessorLevel) + 'A');
		  }
		  else rev = (float)(siSysInfo.wProcessorLevel & 0x00FF);
		break;
	  case PROCESSOR_ARCHITECTURE_MIPS:
		strcpy(vendor, "MIPS");
		break;
	  case PROCESSOR_ARCHITECTURE_ALPHA:
		strcpy(vendor, "Compaq Alpha");
		break;
	  case PROCESSOR_ARCHITECTURE_PPC:
		strcpy(vendor, "Power PC");
		break;
	  ...
	  case PROCESSOR_ARCHITECTURE_IA64:
		strcpy(vendor, "Intel Itanium 64");
		break;
	  case PROCESSOR_ARCHITECTURE_ALPHA64:
		strcpy(vendor, "Compaq Alpha 64");
		break;
...
static int get_system_info(const struct vperfctr *dev)
{
  ...
  _papi_system_info.hw_info.ncpu = (long)siSysInfo.dwNumberOfProcessors;
  _papi_system_info.hw_info.nnodes = 1;

  activeProcessorMask = siSysInfo.dwActiveProcessorMask;
  while (activeProcessorMask) {
	i += (activeProcessorMask & 1);
	activeProcessorMask = activeProcessorMask >> 1;
  }
  _papi_system_info.hw_info.totalcpus = i;
  _papi_system_info.hw_info.vendor = siSysInfo.wProcessorArchitecture;

  winRevision(siSysInfo, _papi_system_info.hw_info.vendor_string,
		_papi_system_info.hw_info.revision);
  ...
  _papi_system_info.hw_info.mhz = (float)info.cpu_khz / (float)1000; 
...
inline_static int counter_event_shared(const struct perfctr_control *a, const struct perfctr_control *b, int cntr)
{
...
inline_static int counter_event_compat(const struct perfctr_control *a, const struct perfctr_control *b, int cntr)
{
...
inline_static void counter_event_copy(const struct perfctr_control *a, struct perfctr_control *b, int cntr)
{
...
{
	// supposed to return time per process as defined in sys/times.h
	// but the code below returns time per current thread
...
long_long _papi_hwd_get_virt_cycles (EventSetInfo *zero)
{
  ...
  usec = (float)_papi_hwd_get_virt_cycles(zero);
