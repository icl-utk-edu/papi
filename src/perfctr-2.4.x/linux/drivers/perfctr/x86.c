...
static inline int __init p4_ht_finalise(unsigned long forbidden)
{
	printk(KERN_WARNING "perfctr/x86.c: hyper-threaded P4s detected:"
	       " you need a newer kernel! -- bailing out\n");
	return -ENODEV;
}
#else
...
static inline int __init p4_ht_finalise(unsigned long forbidden)
{
	...
	printk(KERN_INFO "perfctr/x86.c: hyper-threaded P4s detected:"
	       " restricting access for CPUs 0x%lX\n", forbidden);
	return 0;
}

#endif

static void __init p4_ht_mask_setup_cpu(void *forbiddenp)
{
	...
	if( logical_processor_id != 0 ) {
		...
		atomic_set_mask(bit, (unsigned long*)forbiddenp);
	}
}
...
static int __init p4_ht_init(void)
{
	...
	if( !forbidden )
		return 0;
	return p4_ht_finalise(forbidden);
}
#else /* SMP */
#define p4_ht_init()	(0)
#endif /* SMP */
