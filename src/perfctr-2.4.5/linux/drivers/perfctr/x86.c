...
static void p4_isuspend(struct perfctr_cpu_state *state)
{
	...
	for(i = perfctr_cstatus_nractrs(cstatus); i < nrctrs; ++i) {
		...
		/* Erratum N17 does not apply since we read only low 32 bits. */
		rdpmcl(pmc_raw, now);
		state->sum.pmc[i] += now - state->start.pmc[i];
		state->start.pmc[i] = now;
	}
...
static void p4_iresume(const struct perfctr_cpu_state *state)
{
	...
	for(i = perfctr_cstatus_nractrs(cstatus); i < nrctrs; ++i) {
		...
		if( cpu->control.evntsel[pmc] ) {
			cpu->control.evntsel[pmc] = 0;
			wrmsr(MSR_P4_CCCR0+pmc, 0, 0);
		}
		/* Erratum N15 does not apply since the CCCR is disabled. */
...
static void p4_write_control(const struct perfctr_cpu_state *state)
{
	...
	if( (perfctr_cpus_forbidden_mask & (1 << smp_processor_id())) &&
...
static inline void set_perfctr_cpus_forbidden_mask(unsigned long mask)
{
...
static void __init finalise_backpatching(void)
{
	...
	set_perfctr_cpus_forbidden_mask(0);
...
static inline int __init p4_ht_mask_setup(void)
{
	printk(KERN_WARNING "perfctr/x86.c: hyper-threaded P4s detected:"
	       " kernel too old! -- bailing out\n");
	return -ENODEV;
}
#else

unsigned long perfctr_cpus_forbidden_mask;

static void __init p4_ht_mask_setup_cpu(void *ignore)
{
	...
	if( logical_processor_id != 0 ) {
		unsigned long bit = 1 << smp_processor_id();
		atomic_set_mask(bit, &perfctr_cpus_forbidden_mask);
	}
}

static inline int __init p4_ht_mask_setup(void)
{
	smp_call_function(p4_ht_mask_setup_cpu, NULL, 1, 1);
	p4_ht_mask_setup_cpu(NULL);
	printk(KERN_INFO "perfctr/x86.c: hyper-threaded P4s detected: restricting"
	       " access for CPUs 0x%lX\n", perfctr_cpus_forbidden_mask);
	return 0;
}
#endif
...
static int __init p4_ht_init(void)
{
	...
	return (nr_siblings < 2) ? 0 : p4_ht_mask_setup();
}
#else /* SMP */
#define p4_ht_init()	(0)
#endif /* SMP */
