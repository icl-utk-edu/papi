...
int __init gperfctr_init(void)
{
	...
	/* XXX: current API cannot handle CPU affinity restrictions */
	if( perfctr_cpus_forbidden_mask ) {
		printk(KERN_ERR "gperfctr: asymmetric MP not yet supported\n");
		return 0; /* XXX: init.c treats error as fatal :-( */
	}
