...
#include "errno.h"
	...
	_papi_hwi_system_info.hw_info.nnodes =
		BGLPersonality_numNodesInPset( &bgl );
...
inline_static int
setup_bgl_presets( int cpu_type )
{
	switch ( cpu_type ) {
...
mdi_init(  )
{
	...
	strcpy( _papi_hwi_system_info.sub_info.name,
			"$Id: linux-bgl.c,v 1.00 2005/07/01 20:43:32 noeth2" );
...
int
_papi_hwd_add_prog_event( hwd_control_state_t * state, unsigned int code,
						  void *tmp, EventInfo_t * tmp2 )
{
	return ( PAPI_ESBSTR );
}
...
int
_papi_hwd_bpt_map_avail( hwd_reg_alloc_t * dst, int ctr )
{
...
/* This function forces the event to
    ...
    Returns nothing.  */
void
_papi_hwd_bpt_map_set( hwd_reg_alloc_t * dst, int ctr )
{
...
int
_papi_hwd_bpt_map_exclusive( hwd_reg_alloc_t * dst )
{
...
int
_papi_hwd_bpt_map_shared( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
...
/* This function removes shared resources available to the src event
    ...
    Returns nothing.  */
void
_papi_hwd_bpt_map_preempt( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	int i;
	unsigned shared;
...
void
_papi_hwd_bpt_map_update( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	dst->ra_selector = src->ra_selector;
}

int
check_added_event( BGL_PERFCTR_event_t event, hwd_control_state_t * this_state )
{
	bgl_perfctr_control_t *bst = &this_state->perfctr;
	int i;

	for ( i = 0; i < bst->nmapped; i++ ) {
		if ( bst->map[i].event.num == event.num &&
			 bst->map[i].event.edge == event.edge )
			return 1;
	}
	return 0;
}
...
int
update_control_state( hwd_control_state_t * ctrlstate )
{
	int err;

	err =
		bgl_perfctr_copy_state( &ctrlstate->perfctr,
								sizeof ( bgl_perfctr_control_t ) );
	if ( err ) {
		return ( err );
	}
...
/* Register allocation */
int
_papi_hwd_allocate_registers_foo( EventSetInfo_t * ESI )
{
	...
	for ( i = 0; i < natNum; i++ ) {
		...
		SUBDBG
			( "---ctx->perfstate(%p): nmapped = %d  event.num = 0x%x   event.edge=%d\n",
...
static int
get_register( int EventCode, hwd_control_state_t * current_state )
{
	int i;
	BGL_PERFCTR_event_t event;
...
static int
check_and_update_events( bgl_perfctr_control_t * perfstate,
								...
								hwd_context_t * ctx )
{
	...
	SUBDBG
		( "===================_papi_hwd_update_control_state(%p) %d native events\n",
		  perfstate, count );
...
/* inline */ static int
update_global_hwcounters( hwd_context_t * ctx )
{
...
/* remove event which does not exist in native from perfstate, and add events that mapped */
static int
swap_events( bgl_perfctr_control_t * perfstate,
			 bgl_perfctr_control_t * currentstate )
{
	int i, j, err = 0;
	...
	for ( j = 0; j < currentstate->nmapped; j++ ) {
		...
		if ( i == perfstate->nmapped &&
			 currentstate->map[j].event.num != BGL_PAPI_TIMEBASE ) {
...
int
_papi_hwd_start( hwd_context_t * ctx, hwd_control_state_t * ctrlstate )
{
	int retval;
...
int
_papi_hwd_read( hwd_context_t * ctx, hwd_control_state_t * this_state,
...
int
_papi_hwd_reset( hwd_context_t * ctx, hwd_control_state_t * ctrlstate )
{
...
int
_papi_hwd_write( hwd_context_t * ctx, hwd_control_state_t * cntrl,
				 long long *from )
{
	return ( PAPI_ESBSTR );
}
...
void
_papi_hwd_dispatch_timer( int signal, hwd_siginfo_t * si, void *context )
{
}
...
papi_svector_t _bgl_svector_table[] = {
	...
	{( void ( * )(  ) ) _papi_hwd_shutdown_substrate,
...
static inline void
internal_decode_event( unsigned int EventCode, int *event )
{
