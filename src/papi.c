   ...
   APIDBG("Entry: in: %p, name: %s, out: %p\n", in, in, out);
	...
	if (strncmp(in, "PAPI_", 5) == 0) {
       ...
       if( NULL == evt_name_copy ) {
           fprintf(stdout, "evt_name_copy is NULL\n");
           fflush(stdout);
		   papi_return( PAPI_EINVAL );
       }
       ...
       if( NULL == evt_base_name ) {
           fprintf(stdout, "evt_base_name is NULL\n");
           fflush(stdout);
		   papi_return( PAPI_EINVAL );
       }
       ...
       if( NULL != evt_base_name ) {
           ...
           fprintf(stdout, "premature return 1: NULL\n");
           fflush(stdout);
	       papi_return( PAPI_EINVAL );
       }
       ...
       if( NULL != _papi_hwi_list ) {
	       ...
	       for(j = 0; j < _papi_hwi_list[preset_idx].count; j++ ) {
               ...
               if( qual_str == NULL ) {
                   ...
                   retval = sprintf(new_ntv_name, "%s", _papi_hwi_list[preset_idx].base_name[j]);
               } else {
                   len += strlen(qual_str);
                   new_ntv_name = (char*)malloc(len*sizeof(char));
                   retval = sprintf(new_ntv_name, "%s%s", _papi_hwi_list[preset_idx].base_name[j], qual_str);
...
PAPI_enum_event( int *EventCode, int modifier )
{
    ...
    if ( IS_PRESET(i) ) {
        ...
        if ( modifier == PAPI_ENUM_EVENTS || modifier == PAPI_PRESET_ENUM_AVAIL ) {
            ...
            do {
                ...
                if ( modifier == PAPI_PRESET_ENUM_AVAIL
                     && _papi_hwi_list[preset_index].count == 0 ) {
                    continue;
                }
        ...
        if ( modifier == PAPI_ENUM_FIRST_COMP ) {
