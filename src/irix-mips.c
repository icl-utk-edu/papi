...
int _papi_hwd_add_event(void *machdep, int event)
{
  ...
  if (foo & PRESET_MASK)
  ...
  else
    {
      if ((event >= 0) && (event <= 15))
	{
	  if (this_state->counter_code1 == UNUSED)
	    {
	...
	{
	  if (this_state->counter_code2 == UNUSED)
	    {
...
int _papi_hwd_rem_event(void *machdep, int event)
{
  ...
  if (foo & PRESET_MASK)
    { 
      preset = foo ^= PRESET_MASK; 
      if (preset_map[preset].counter_code1 != UNUSED)
	this_state->counter_code1 = UNUSED;
      if (preset_map[preset].counter_code2 != UNUSED)
	this_state->counter_code2 = UNUSED;
      return(PAPI_OK);
    }
  else
    {
