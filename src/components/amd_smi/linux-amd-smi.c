...
// hardware system management controls for AMD GPU devices through the
// AMD SMI library.
//-----------------------------------------------------------------------------
...
static int
_amd_smi_check_n_initialize(void)
{
...
static int
_amd_smi_init_thread(hwd_context_t *ctx)
{
...
static int
_amd_smi_init_component(int cidx)
{
...
_amd_smi_init_private(void)
{
    ...
    papi_errno = amds_init();
    if (papi_errno != PAPI_OK) {
        _amd_smi_vector.cmp_info.disabled = papi_errno;
        const char *error_str;
        amds_err_get_last(&error_str);
        sprintf(_amd_smi_vector.cmp_info.disabled_reason, "%s", error_str);
        goto fn_fail;
    }
...
static int
_amd_smi_init_control_state(hwd_control_state_t *ctrl)
{
    (void) ctrl;  // unused
...
static int
_amd_smi_update_control_state(hwd_control_state_t *ctrl, NativeInfo_t *nativeInfo, int nativeCount, hwd_context_t *ctx)
{
...
static int
update_native_events(amdsmi_control_t *ctl, NativeInfo_t *ntvInfo, int ntvCount)
{
    int papi_errno = PAPI_OK;
...
static int
try_open_events(amdsmi_control_t *ctl)
{
    // No pre-opening needed in this design (events opened on start)
    return PAPI_OK;
}

static int
_amd_smi_start(hwd_context_t *ctx, hwd_control_state_t *ctrl)
{
    int papi_errno = PAPI_OK;
    amdsmi_context_t *amdsmi_ctx = (amdsmi_context_t *) ctx;
    amdsmi_control_t *amdsmi_ctl = (amdsmi_control_t *) ctrl;
...
static int
_amd_smi_read(hwd_context_t *ctx, hwd_control_state_t *ctrl, long long **values, int flags)
{
    (void) flags;  // unused
    amdsmi_context_t *amdsmi_ctx = (amdsmi_context_t *) ctx;
    amdsmi_control_t *amdsmi_ctl = (amdsmi_control_t *) ctrl;
...
static int
_amd_smi_write(hwd_context_t *ctx, hwd_control_state_t *ctrl, long long *values)
{
    amdsmi_context_t *amdsmi_ctx = (amdsmi_context_t *) ctx;
    amdsmi_control_t *amdsmi_ctl = (amdsmi_control_t *) ctrl;
    if (!(amdsmi_ctx->state & AMDS_EVENTS_RUNNING)) {
...
static int
_amd_smi_stop(hwd_context_t *ctx, hwd_control_state_t *ctrl)
{
    int papi_errno = PAPI_OK;
    amdsmi_context_t *amdsmi_ctx = (amdsmi_context_t *) ctx;
    amdsmi_control_t *amdsmi_ctl = (amdsmi_control_t *) ctrl;
    ...
    papi_errno = amds_ctx_stop(amdsmi_ctl->amds_ctx);
    if (papi_errno != PAPI_OK) {
        return papi_errno;
    }
    amdsmi_ctx->state &= ~AMDS_EVENTS_RUNNING;
...
static int
_amd_smi_reset(hwd_context_t *ctx, hwd_control_state_t *ctrl)
{
    amdsmi_context_t *amdsmi_ctx = (amdsmi_context_t *) ctx;
    amdsmi_control_t *amdsmi_ctl = (amdsmi_control_t *) ctrl;
    if (!(amdsmi_ctx->state & AMDS_EVENTS_RUNNING)) {
...
static int
_amd_smi_cleanup_eventset(hwd_control_state_t *ctrl)
{
    amdsmi_control_t *amdsmi_ctl = (amdsmi_control_t *) ctrl;
    if (amdsmi_ctl->amds_ctx != NULL) {
        // Context should be NULL if properly stopped
        return PAPI_EMISC;
    }
...
static int
_amd_smi_shutdown_thread(hwd_context_t *ctx)
{
    amdsmi_context_t *amdsmi_ctx = (amdsmi_context_t *) ctx;
    amdsmi_ctx->state = 0;
...
static int
_amd_smi_shutdown_component(void)
{
    if (!_amd_smi_vector.cmp_info.initialized) {
        return PAPI_EMISC;
    }
...
static int
_amd_smi_ctrl(hwd_context_t *ctx, int code, _papi_int_option_t *option)
{
    (void) ctx; (void) code; (void) option;
    // No special control actions needed
    return PAPI_OK;
}

static int
_amd_smi_set_domain(hwd_control_state_t *ctrl, int domain)
{
    (void) ctrl; (void) domain;
    // This component only supports the default domain (user|kernel)
    return PAPI_OK;
}

static int
_amd_smi_ntv_enum_events(unsigned int *EventCode, int modifier)
{
    int papi_errno = _amd_smi_check_n_initialize();
    if (papi_errno != PAPI_OK) {
        return papi_errno;
...
static int
_amd_smi_ntv_code_to_name(unsigned int EventCode, char *name, int len)
{
    int papi_errno = _amd_smi_check_n_initialize();
    if (papi_errno != PAPI_OK) {
        return papi_errno;
...
static int
_amd_smi_ntv_name_to_code(const char *name, unsigned int *EventCode)
{
    int papi_errno = _amd_smi_check_n_initialize();
    if (papi_errno != PAPI_OK) {
        return papi_errno;
...
static int
_amd_smi_ntv_code_to_descr(unsigned int EventCode, char *desc, int len)
{
    int papi_errno = _amd_smi_check_n_initialize();
    if (papi_errno != PAPI_OK) {
        return papi_errno;
