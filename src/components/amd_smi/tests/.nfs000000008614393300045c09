...
// make -f AMD_SMI_Makefile amd_smi_all.out
//-----------------------------------------------------------------------------
...
{
    printf("Demonstrate use of AMD SMI APIs\n");
    printf("This program has no options, it will attempt to try every combination of amd_smi PAPI events\n");
...
int main(int argc, char *argv[])
{
    ...
    for (i=0; i<k && cid<0; i++) {                                     // while not found,
       ...
       if (strcmp("amd_smi", aComponent->name) == 0) cid=i;           // If we found our match, record it.
    } // end search components.

    if (cid < 0) {                                                     // if no amd_smi component found,
        fprintf(stderr, "Failed to find amd_smi component among %i "
                "reported components.\n", k);
        FreeGlobals();
        PAPI_shutdown();
        exit(-1);
    ...
    printf("Found AMD_SMI Component at id %d\n", cid);

    // Add events at a GPU specific level ... eg amd_smi:::device=0:Whatever
    eventCount = 0;
    int eventsRead=0;
...
//              fprintf(stderr, "Found amd_smi symbol '%s', device=%i.\n", info.symbol , device);
                if (device < 0 || device >= 32) continue;                  // skip any not in range.
            } else {                                                   // A few are system wide.
//              fprintf(stderr, "Found amd_smi symbol '%s'.\n", info.symbol);
            ...
            // Filter out crashers.
            // NOTE: These filters were for rocm_smi. Review and update for amd_smi if needed.
            /*
            ...
            // if (strstr(info.symbol, "temp_emergency:device=0:sensor=3") != NULL) continue; // Duplicate line in original
            ...
            // if (strstr(info.symbol, "temp_emergency:device=1:sensor=3") != NULL) continue; // Duplicate line in original
            */
...
//  fprintf(stderr, "%s:%i Finished Event Loops.\n", __FILE__, __LINE__);
    ...
    if (eventCount < 1) {                                              // If we failed on all of them,
        fprintf(stderr, "Unable to add any AMD_SMI events; they are not present in the component.\n");
...
//              fprintf(stderr, "readValues[0]=%lli readValues[1]=%lli.\n", readValues[0], readValues[1]);
                goodOnSame++;                                                     // Was accepted by PAPI as a valid pairing.
