...
int main(int argc, char *argv[]) {
    ...
    if( cap == overall_status ) {
        test_fail(__FILE__, __LINE__, "one of the threads", PAPI_EMISC);
    }
...
void *thread_work(void *data) {
    ...
    for(i = id; i < total_event_count; i+=ONT) {
        ...
        if( PAPI_OK != stat ) {
            fprintf(stdout, "Thread %d: Failed to add event %s to set. [%d]\n", id, rocp_sdk_native_event_names[i], stat);
            status[id] = stat;
        } else {
            ++numValues;
        }
    ...
    for(g = 0; g < NUMGEMMS; ++g) {
        ...
        if( PAPI_OK != status[id] || PAPI_OK != stat ) {

            fprintf(stdout, "\tError: Failed to read! [%d]\n", stat);
            status[id] = stat;
        } else {
            for(i = id; i < total_event_count; i+=ONT) {
                fprintf(stdout, "\t%s : %lld\n", rocp_sdk_native_event_names[i], values[i/ONT]);
            }
        }
    }
