...
gpconfig="set grid; set key top left; set style line 1 lw 6; set style line 2 lw 1.5; set style line 3 lw 1.5; set style line 4 lw 1.5; set style line 5 lw 1.5; set style line 6 lw 1.5 lc rgb 'pink'; set style line 7 lw 1.5;"
...
plot \
    'libmsr_write_test_output_tmp_set.dat' index 0 using (\$2):(\$9) title "Set/Request Avg Power Cap (watts in 1 sec)" smooth unique with points ls ${ls_1},\
    'libmsr_write_test_output_tmp_read.dat' index 0 using (\$2):(\$6) title "Read Power Consumpution (watts)" smooth unique with linespoints ls ${ls_2}, \
