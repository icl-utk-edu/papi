		 ...
		 int flags )
{
	...
	for ( i = 0; i < MX_MAX_COUNTERS; i++ ) {
		( ( MX_control_state_t * ) ctrl )->counts[i] =
			_papi_hwd_mx_register[i] - _papi_hwd_mx_register_start[i];
		/*printf("%d  %lld\n", i, ctrl->counts[i]); */
	}
...
int
MX_ntv_code_to_name( unsigned int EventCode, char *name, int len )
{
	strncpy( name,
			 mx_native_table[EventCode & PAPI_NATIVE_AND_MASK &
							 PAPI_COMPONENT_AND_MASK].name, len );
	return ( PAPI_OK );
}

int
MX_ntv_code_to_descr( unsigned int EventCode, char *name, int len )
{
	strncpy( name,
			 mx_native_table[EventCode & PAPI_NATIVE_AND_MASK &
...
int
MX_ntv_code_to_bits( unsigned int EventCode, hwd_register_t * bits )
{
...
/* Register allocation */
int
MX_allocate_registers( EventSetInfo_t * ESI )
{
	int i, natNum;
