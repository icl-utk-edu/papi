    ...
    for(ii = 0; ii < nativeCount; ii++) {
        ...
        case CUPTI_ACTIVITY_KIND_METRIC:
            ...
            for(ee = 0; ee < numEvents; ee++) {
                ...
                CHECK_PRINT_EVAL(eventctrl->conEventsCount >= PAPICUDA_MAX_COUNTERS, "Num events (generated by metric) exceeded PAPICUDA_MAX_COUNTERS", return (PAPI_ENOMEM));
            }
            eventctrl->conMetrics[eventctrl->conMetricsCount] = metricId;
            eventctrl->conMetricsCount++;
            CHECK_PRINT_EVAL(eventctrl->conMetricsCount >= PAPICUDA_MAX_COUNTERS, "Num metrics exceeded PAPICUDA_MAX_COUNTERS", return (PAPI_ENOMEM));
            break;
        ...
        CHECK_PRINT_EVAL(eventctrl->conEventsCount >= PAPICUDA_MAX_COUNTERS, "Num events exceeded PAPICUDA_MAX_COUNTERS", return (PAPI_ENOMEM));
