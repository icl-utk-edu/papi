...
static int cuda_ntv_name_to_code(const char *name, unsigned int *event_code)
{
    ...
    ntv_event_t *evt_rec;
    papi_errno = cuptid_event_table_find_name(global_event_names, name, (void**) &evt_rec);
    if (papi_errno == PAPI_OK) {
...
static int cuda_ntv_code_to_name(unsigned int event_code, char *name, int len)
{
    ...
    ntv_event_t *evt_rec;
    papi_errno = cuptid_event_table_get_item(global_event_names, event_code, (void **) &evt_rec);
    if (papi_errno != PAPI_OK) {
        return PAPI_ENOEVNT;
    }
