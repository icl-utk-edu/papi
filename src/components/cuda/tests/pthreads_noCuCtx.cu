...
int main(int argc, char **argv)
{
    ...
    if (global_total_event_count == 0) {
        global_cuda_native_event_names[global_total_event_count] = (char *) malloc(PAPI_MAX_STR_LEN * sizeof(char));
        if (global_cuda_native_event_names[global_total_event_count] == NULL) {
            fprintf(stderr, "Failed to allocate memory for index %d for the metric_names array.\n", global_total_event_count);
            exit(EXIT_FAILURE);
        }
