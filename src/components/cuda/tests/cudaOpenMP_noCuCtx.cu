...
*        cudaSetDevice will be called only for enabled devices. cudaSetDevice determines
*        which device executions will be done on.
*
...
{
    printf("./cudaOpenMP_noCuCtx --cuda-native-event-names [list of cuda native event names separated by a comma].\n"
           "Notes:\n"
...
int main(int argc, char *argv[])
{
    ...
    if( papi_errno != PAPI_VER_CURRENT ) {
        test_fail(__FILE__, __LINE__, "PAPI_library_init()", 0);
    }
    ...
    int thread_and_dev_idx, event_idx, numMultipassEvents = 0;
