/**
* @file simpleMultiGPU_noCuCtx.cu
* @brief For all enabled NVIDIA devices detected on the machine a matching call to cudaSetDevice
*        will be made. cudaSetDevice determines which device executions will be done on.
*
...
{
    printf("./simpleMultiGPU_noCuCtx --cuda-native-event-names [list of cuda native event names separated by a comma].\n"
           "Notes:\n"
...
int main(int argc, char **argv)
{
    ...
    char **cuda_native_event_names = (char **) malloc(1 * sizeof(char *));
    ...
    if (total_event_count == 0) {
        cuda_native_event_names[total_event_count] = (char *) malloc(PAPI_MAX_STR_LEN * sizeof(char));
    ...
    // Process GPU results
    PRINT(suppress_output, "Process GPU results on %d GPUs...\n", num_devices);
    for(dev_idx = 0; dev_idx < num_devices; dev_idx++) {
        if (cmpInfo->partially_disabled) {
            // Device is not enabled continue
            if (determine_if_device_is_enabled(dev_idx) == 0) {
