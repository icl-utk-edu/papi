...
*        were running while native events were gathered and the user replay mechanism was in use. cudaSetDevice
*        is used to set the device which device executions will be done on.
*
...
{
    printf("./concurrent_profiling_noCuCtx\n");
    printf("Notes:\n"
           "1. This test is specifically designed to use devices that support CUPTI Profiling i.e. devices with CCs >= 7.0.\n"
           "2. No events are accepted from the command line as cuda:::sm_cycles_active:stat=sum and cuda:::sm__cycles_elapsed:stat=max\n"
           "   are required events.\n");
}
...
int main(int argc, char **argv)
{
    ...
    PRINT(global_suppress_output, "Running the cuda component test concurrent_profiling_noCuCtx.cu\n")
