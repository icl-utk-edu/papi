...
char **global_cuda_native_event_names;
...
{
    printf("./test_2thr_1gpu_not_allowed --cuda-native-event-names [list of cuda native event names separated by a comma].\n"
           "Notes:\n"
           "1. Native event names must not have the device qualifier appended.\n"
           "2. Must provide exactly two native event names on the command line.\n");
}
    ...
    {   
        ...
        {   
            ...
            while (cuda_native_event_name != NULL)
            {
                ...
                int strLen = snprintf(cuda_native_event_names[(*total_event_count)], PAPI_MAX_STR_LEN, "%s", cuda_native_event_name);
...
void *thread_gpu(void * ptinfo)
{
    ...
    int gpuid = thread_idx % global_num_devices;
    unsigned long gettid = (unsigned long) pthread_self();

    check_cuda_driver_api_call( cuCtxSetCurrent(tinfo->cuCtx) );

    PRINT(global_suppress_output, "This is idx %d thread %lu - using GPU %d context %p!\n",
          thread_idx, gettid, gpuid, tinfo->cuCtx);
    ...
    if (papi_errno == PAPI_ECNFLCT) {
        PRINT(global_suppress_output, "Thread %d was not allowed to start profiling on same GPU.\n", thread_idx);
...
int main(int argc, char **argv)
{
    ...
    global_cuda_native_event_names = (char **) malloc(1 * sizeof(char *));
    ...
    if (global_total_event_count == 0) {
        ...
        while (PAPI_enum_cmp_event(&cuda_eventcode, modifier, cuda_cmp_idx) == PAPI_OK && global_total_event_count != 2) {
            ...
            global_cuda_native_event_names[global_total_event_count] = (char *) calloc(1, PAPI_MAX_STR_LEN * sizeof(char));
            check_memory_allocation_call( global_cuda_native_event_names[global_total_event_count] );

            int strLen = snprintf(global_cuda_native_event_names[global_total_event_count], PAPI_MAX_STR_LEN, "%s", basename);
            if (strLen < 0 || strLen >= PAPI_MAX_STR_LEN) {
            ...
            do {
                ...
                int strLen = snprintf(global_cuda_native_event_names[global_total_event_count] + strlen(global_cuda_native_event_names[global_total_event_count]), PAPI_MAX_STR_LEN - strlen(global_cuda_native_event_names[global_total_event_count]), "%s", qualifier);
                if (strLen < 0 || strLen >= PAPI_MAX_STR_LEN) {
    ...
    for(thread_idx = 0; thread_idx < NUM_THREADS; thread_idx++)
    {
        ...
        PRINT(global_suppress_output, "\n Main thread %lu. Created new thread (%lu) in iteration %d ...\n",
              (unsigned long)pthread_self(), (unsigned long) data[thread_idx].tid, thread_idx);
    }
    ...
    for (thread_idx = 0; thread_idx < NUM_THREADS; thread_idx++) {
        ...
        PRINT(global_suppress_output, "IDX: %d: TID: %lu: Done! Joined main thread.\n", thread_idx, (unsigned long)data[thread_idx].tid);
    }
