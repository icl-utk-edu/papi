...
static void print_help_message(void)
{
    printf("./test_2thr_1gpu_not_allowed --device [nvidia device index] --cuda-native-event-names [list of cuda native event names separated by a comma].\n"
           "Notes:\n"
           "1. Both args (--device and --cuda-native-event-names) must be provided.\n"
           "2. Must provide exactly two native event names on the command line and if the device qualifier's are appended then is must match the device index provided to --device.\n");
}
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1) {
        ...
        if (global_total_event_count != 2) {
            fprintf(stderr, "Must provide two single pass Cuda native events on the command line for this test to run properly.\n");
            test_skip(__FILE__, __LINE__, "", 0);
        }
    }
