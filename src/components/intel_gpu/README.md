...
* If the i915 driver is in use, then `/proc/sys/dev/i915/perf_stream_paranoid` must be set to `0` (`sudo sysctl dev.i915.perf_stream_paranoid=0`).
* Alternatively, if the Xe driver is in use, then `/proc/sys/dev/xe/observation_paranoid` must be set to `0` (`sudo sysctl dev.xe.observation_paranoid=0`).
