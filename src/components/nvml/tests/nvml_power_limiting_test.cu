...
#define NUM_EVENTS 32 /* Max number of GPUs on a node this code can handle. */
   ...
   int cid = 0;
    ...
    if ( cid==numcmp ) {                                        // If true we looped through all without finding nvml.
        printf("NVML PAPI Component was not found.\n");       
    ...
    if (cmpinfo->disabled) {                                    // If disabled,
        printf("NVML PAPI Component is disabled.\n");
    ...
    for (i=0; i< eventCount; i++) {
        ...
        if ( retval!=PAPI_OK ) {
            ...
            test_skip( __FILE__,__LINE__,"Attempted PAPI_write of power_management_limit failed:  Possible reasons: Insufficient permissions; Power management unavailable;. Outside min/max limits; failed to run with sudo.", retval );
        } else {
        ...
        if( retval != PAPI_OK ) fprintf( stderr, "PAPI_read for reset to original value failed.\n" );
        
        if ( values[i] != initial_power_management_limit) {
            printf( "Mismatch on reset: power_management_limit on device %d set to %llu but read as %llu\n", device_id[i], initial_power_management_limit, values[i] );
            for (j=0; j<eventCount; j++) free(EventName[j]);            // clean up memory.
            PAPI_stop(EventSet, values);                                // Must be stopped.
            PAPI_cleanup_eventset(EventSet);                            // Empty it.
            PAPI_destroy_eventset(&EventSet);                           // Release memory.
