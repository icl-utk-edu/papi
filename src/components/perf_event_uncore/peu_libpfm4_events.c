...
static struct native_event_t *allocate_native_event(char *name, int libpfm4_index,
  ...
  int new_event_code;
  if(strlen(masks) == 0) {
	new_event_code = _papi_hwi_native_to_eventcode(_peu_libpfm4_get_cidx(), libpfm4_index, -1, ntv_evt->allocated_name);
  } else {
	// we have a mask, use the correct index into the native events array for the index
	new_event_code = _papi_hwi_native_to_eventcode(_peu_libpfm4_get_cidx(), libpfm4_index, nevt_idx, ntv_evt->allocated_name);
  }
...
_peu_libpfm4_ntv_enum_events( unsigned int *PapiEventCode,
	...
	if ( modifier == PAPI_ENUM_FIRST ) {
	   ...
	   if (code < 0 ) {
		...
		int new_event_code = _papi_hwi_native_to_eventcode(_peu_libpfm4_get_cidx(), code, -1, einfo.name);
		_papi_hwi_set_papi_event_string(einfo.name);
		_papi_hwi_set_papi_event_code(new_event_code, 1);
		*PapiEventCode = code;
	...
	if ( modifier == PAPI_ENUM_EVENTS ) {
		...
		_peu_libpfm4_ntv_code_to_name(code, event_string, sizeof(event_string), event_table);
...
//      SUBDBG("*PapiEventCode: %#x, einfo.name: %s, einfo.code: %#x, einfo.nattrs: %d\n", *PapiEventCode, einfo.name, einfo.code, einfo.nattrs);
		...
		// get the event name for this event code
		_peu_libpfm4_ntv_code_to_name(*PapiEventCode, event_string, sizeof(event_string), event_table);
