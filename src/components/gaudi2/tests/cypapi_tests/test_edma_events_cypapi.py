...
Test script for PAPI Gaudi2 component EDMA (Engine DMA) SPMU events
using cyPAPI. Exercises EDMA-specific counters on a single Gaudi2 device
using a PyTorch workload that triggers internal memory transfers (EDMA
handles HBM-to-HBM data movement within the device).
...
Usage:
    PT_HPU_LAZY_MODE=1 python3 test_edma_events_cypapi.py
"""
...
def run_edma_workload():
    """Run a workload that exercises EDMA (internal memory copies)"""
    ...
    try:
        ...
        # Large tensor copies trigger EDMA transfers within HBM
        a = _torch.randn(size, size, dtype=_torch.float32, device=_device)
        _hthpu.synchronize()

        # Warm-up
        for _ in range(3):
            b = a.clone()
            c = _torch.cat([a, b], dim=0)
        _hthpu.synchronize()

        # Actual workload - repeated copies and reshapes to drive EDMA
        for _ in range(20):
            b = a.clone()
            c = _torch.cat([a, b], dim=0)
            d = c.transpose(0, 1).contiguous()
        _hthpu.synchronize()
...
def main():
    ...
    # Step 3: Add EDMA events
    print("\n[3] Adding EDMA events...")
    events = [
        "gaudi2:::EDMA_DESC_PUSH:device=0",
        "gaudi2:::EDMA_DMA_TRC_DESC_PUSH:device=0",
        "gaudi2:::EDMA_DMA_TRC_CPL_MSG_SENT:device=0",
        "gaudi2:::EDMA_MESH2SB_BP:device=0",
        "gaudi2:::EDMA_SB2MESH_BP:device=0",
        "gaudi2:::EDMA_TRACE_FENCE_DONE:device=0",
    ]
    ...
    if len(added_events) == 0:
        print("\n  ERROR: No EDMA events added. Is the gaudi2 component built?")
    ...
    print(f"\n[4] Starting counters ({len(added_events)} EDMA events)...")
    try:
        eventset.start()
    except Exception as e:
    ...
    # Step 5: Run workload (copies and transposes to exercise EDMA)
    print("\n[5] Running EDMA workload (tensor copies/transposes, 20 iterations)...")
    if not run_edma_workload():
