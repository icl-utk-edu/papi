...
Test script for PAPI Gaudi2 component MME (Matrix Math Engine) SPMU events
using cyPAPI. Exercises MME-specific counters on a single Gaudi2 device
using a PyTorch matrix multiplication workload which exercises the MME.
...
Usage:
    PT_HPU_LAZY_MODE=1 python3 test_mme_events_cypapi.py
"""
...
def run_matmul_workload():
    """Run a PyTorch matmul workload that exercises the MME"""
    ...
    try:
        ...
        # Actual workload - larger iterations to generate more MME activity
        for _ in range(20):
            c = _torch.matmul(a, b)
        _hthpu.synchronize()
...
def main():
    ...
    # Step 3: Add MME events
    print("\n[3] Adding MME events...")
    events = [
        "gaudi2:::MME_NUM_OUTER_PRODUCTS:device=0",
        "gaudi2:::MME_STALL_ON_A:device=0",
        "gaudi2:::MME_STALL_ON_B:device=0",
        "gaudi2:::MME_CONV_END_STALL_ACC:device=0",
        "gaudi2:::MME_QM_PREFETCH_BUFFER_EMPTY:device=0",
        "gaudi2:::MME_PQ_NOT_EMPTY_BUT_CQ_EMPTY:device=0",
    ]
    ...
    if len(added_events) == 0:
        print("\n  ERROR: No MME events added. Is the gaudi2 component built?")
    ...
    print(f"\n[4] Starting counters ({len(added_events)} MME events)...")
    try:
        eventset.start()
    except Exception as e:
    ...
    # Step 5: Run matmul workload (exercises MME heavily)
    print("\n[5] Running PyTorch matmul workload (2048x2048, 20 iterations)...")
    if not run_matmul_workload():
