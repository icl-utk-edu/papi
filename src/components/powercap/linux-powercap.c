...
static int num_packages=0,num_cpus=0,num_zones=0,num_subzones=0,num_attr=10;
...
static long long _local_read_zone_attr(int fd) {

  uint64_t data;

  if ( fd<0 || read(fd, &data, sizeof data) != sizeof data ) {
    perror("rdmsr:pread");
    exit(127);
  }
...
static int _local_get_subzone_count(int zone_id) {
  ...
  if (dir != NULL) {
    ...
    while (entry != NULL) {
      ...
      if (zone_id == -1) { snprintf(buffer, sizeof(buffer), "intel-rapl:%d", count); }
      else { snprintf(buffer, sizeof(buffer),"intel-rapl:%d:%d", zone_id, count); }

      if (!strncmp(entry->d_name, buffer, strlen(buffer))) { count++; }
        entry = readdir(dir);
      }
    closedir(dir);
  }
...
static int _local_write_attr_value(int zone_id, int subzone_id, int which, long long value) {
  ...
  _local_write_zone_str(fd, value_str, PAPI_MAX_STR_LEN);
   ...
   int ret = _local_read_attr_value_str(zone_id,
                                        subzone_id,
                                        which,
...
static int _local_create_powercap_event_name(int zone_id, 
  ...
  switch(attr_id) {
    ...
    case ZONE_ENABLED:
      ...
      break;
    default:
      snprintf(event_name_out, buffsz, "");
      ret = 0;
      break;
  }
  return ret;
 ...
 */
int
_powercap_init_thread( hwd_context_t *ctx )
{
  ( void ) ctx;
 ...
 */
int
_powercap_init_component( int cidx )
{
     int i,j,k,fd;
	...
	PAPI library not done initializing yet */
     ...
     /* get the number of power zones */
     num_zones = _local_get_zone_count();

     /* can't have 0 zones */
     if(num_zones == 0) {
       strncpy(_powercap_vector.cmp_info.disabled_reason,
	       "Intel RAPL module is not supported.",PAPI_MAX_STR_LEN);
       return PAPI_ENOIMPL;
     }
 ...
 */
int 
_powercap_init_control_state( hwd_control_state_t *ctl)
{
...
int 
_powercap_start( hwd_context_t *ctx, hwd_control_state_t *ctl)
{
...
int 
_powercap_stop( hwd_context_t *ctx, hwd_control_state_t *ctl )
{
...
int _powercap_write( hwd_context_t * ctx, hwd_control_state_t * ctl, long long *values )
{
	...
	ee = control->which_counter[nn];
        ...
        if ( event_value_union.dbl == PAPI_NULL ) continue;
        ...
        switch(eventtype) {
   		...
   		break;
                /* read only values */
    	    case ZONE_POWER_LIMIT_A:
                write_powercap_value(nn, event_value_union.ll);
   		break;
    	    case ZONE_TIME_WINDOW_A:
                write_powercap_value(nn, event_value_union.ll);
   		break;
 ...
 */
int 
_powercap_shutdown_component( void ) 
{
    int i,j,k;
...
int
_powercap_update_control_state( hwd_control_state_t *ctl,
 ...
 */
int
_powercap_set_domain( hwd_control_state_t *ctl, int domain )
{
    ( void ) ctl;
...
int
_powercap_reset( hwd_context_t *ctx, hwd_control_state_t *ctl )
{
    ( void ) ctx;
    ( void ) ctl;
 ...
 */
int
_powercap_ntv_enum_events( unsigned int *EventCode, int modifier )
{
 ...
 */
int
_powercap_ntv_code_to_name( unsigned int EventCode, char *name, int len )
{

     int index = EventCode & PAPI_NATIVE_AND_MASK;

     if ( index >= 0 && index < num_events ) {
	strncpy( name, powercap_native_events[index].name, len );
	return PAPI_OK;
     }
     else { printf("failed event: %d\n", index); }
 ...
 */
int
_powercap_ntv_code_to_descr( unsigned int EventCode, char *name, int len )
{
     int index = EventCode;

     if ( index >= 0 && index < num_events ) {
	strncpy( name, powercap_native_events[index].description, len );
	return PAPI_OK;
     }
     return PAPI_ENOEVNT;
}
...
int
_powercap_ntv_code_to_info(unsigned int EventCode, PAPI_event_info_t *info) 
{
  ...
  strncpy( info->symbol, powercap_native_events[index].name, sizeof(info->symbol)-1);
  info->symbol[sizeof(info->symbol)-1] = '\0';

  strncpy( info->long_descr, powercap_native_events[index].description, sizeof(info->long_descr)-1);
  info->long_descr[sizeof(info->long_descr)-1] = '\0';

  strncpy( info->units, powercap_native_events[index].units, sizeof(info->units)-1);
