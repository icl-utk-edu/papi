//-----------------------------------------------------------------------------
// rocm_example.cpp is a minimal example of using PAPI with rocm. It calls a
// function from rocblas; the sgemm routine (single precision general matrix
// multiply), to demonstrate using papi to read a performance counter from an
...
int main( int argc, char **argv )
{
    ...
    char eventname[]="rocm:::SQ_INSTS_VALU:device=0";  // Vector ALU counter.
//  char eventname[]="rocm:::SQ_INSTS_SALU:device=0";  // Scalar ALU counter.
//  char eventname[]="rocm:::SQ_WAVES:device=0";       // Waves counter.
    ...
    rb_sgemm_parms_t *myParms = PrepWork(10240); // set up, param is M,N,K.
    ...
    // Now we are going to read the counters. The function requires the
    // EventSet and a pointer to the long long array for storing the results.
    // Note that PAPI_read() is an interim read, the counters will keep 
    // counting, if more work is done. It is also possible to read and 
    // discontinue the counting, using PAPI_stop(). 
...
void DoWork(rb_sgemm_parms_t *myParms) {
    ...
    fprintf(stderr, "Successful rocblas_sgemm with M,N,K=%d,%d,%d.\n", myParms->m, myParms->n, myParms->k);
    return;
} // end DoWork. 
