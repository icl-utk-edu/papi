/**
 * @file   multi_kernel_monitoring.cpp
 * @author Giuseppe Congiu
 *         gcongiu@icl.utk.edu
 *
 * Single GPU activity monitoring test. This test launches multiple
 * kernels using a for loop. The activity of the two kernels is
 * monitored using PAPI and at the end the counters for the two
 * kernels are read. This test should work seemingly for both sample
 * and intercept mode in the ROCm component.
 */
...
#define KERNEL_BATCH_SIZE (2)
    ...
    long long expected_waves = (long long) (KERNEL_BATCH_SIZE * (ROWS * COLS) / warp_size);
