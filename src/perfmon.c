...
static int generate_native_event_map(hwd_native_event_entry_t **nmap, int *native_cnt, int num_cnt)
{
  int ret, did_something;
  int n, i, j, real_native_cnt = 0;
  size_t max_len;
  ...
  for (i=0;i<*native_cnt;i++)
	...
	{
	  SUBDBG("Could not get an event code for pfm index %d.\n",i);
	  free(findme);
	}
    }
  *nmap = orig_ntmp;
  *native_cnt = real_native_cnt;
  return(PAPI_OK);
}
