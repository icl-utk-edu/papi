...
static void __init do_rdpmc(unsigned unused1, unsigned unused2)
{
	unsigned i;
	for(i = 0; i < NITER/8; ++i)
		__asm__ __volatile__(X8("rdpmc") : : "c"(1) : "eax", "edx");
}
...
measure_overheads(unsigned msr_evntsel0, unsigned evntsel0, unsigned msr_perfctr0)
{
	...
	ticks[2] = run(do_rdmsr, msr_perfctr0, 0);
	name[3] = "rdmsr (evntsel)";
	ticks[3] = run(do_rdmsr, msr_evntsel0, 0);
	name[4] = "wrmsr (counter)";
	ticks[4] = run(do_wrmsr, msr_perfctr0, 0);
	name[5] = "wrmsr (evntsel)";
	ticks[5] = run(do_wrmsr, msr_evntsel0, evntsel0);
	name[6] = "read cr4";
...
{
	unsigned evnt = 0xC0;
	measure_overheads(MSR_P6_EVNTSEL0+1, evnt, MSR_P6_PERFCTR0+1);
}
