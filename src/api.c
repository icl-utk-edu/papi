...
{
  extern const char *_papi_hwi_errNam[PAPI_NUM_ERRORS], *_papi_hwi_errStr[PAPI_NUM_ERRORS];
...
/*      if (ESI->machdep)        free(ESI->machdep); */
  ...
  memset(&ESI->machdep,       0x00,sizeof(hwd_control_state_t));
...
#if 0
static int get_granularity(DynamicArray_t *map, PAPI_granularity_option_t *opt)
{
  EventSetInfo_t *ESI;
...
static int get_free_EventCodeIndex(const EventSetInfo_t *ESI, int EventCode)
{
  ...
  /* You can always add duplicate native events so we don't check those. */

  for (k=0;k<limit;k++) 
    {
      if ((EventCode != PAPI_NATIVE) && (ESI->EventInfoArray[k].event_code == EventCode))
...
int _papi_hwi_lookup_EventCodeIndex(const EventSetInfo_t *ESI, int EventCode)
{
  int i;
  int limit = EventInfoArrayLength(ESI);
  ...
  for(i=0;i<limit;i++) 
    { 
      if ((ESI->EventInfoArray[i].event_code == EventCode) && (EventCode != PAPI_NATIVE))
	return(i);
    }
...
int _papi_hwi_read(hwd_context_t *context, EventSetInfo_t *ESI, unsigned long long *values)
{
  register int i, j = 0;
  int retval;
...
int _papi_hwi_convert_eventset_to_multiplex(EventSetInfo_t *ESI)
{
  ...
  if (ESI->NumberOfEvents)
      ...
      if (mpxlist == NULL)
	{
      ...
      for (i=0;i<EventInfoArrayLength(ESI);i++)
	mpxlist[i] = ESI->EventInfoArray[i].event_code;	      
