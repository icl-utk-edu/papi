...
/* PAPI stuff */
// mpj-fixme -- change back to SUBSTRATE
...
inline_static int setup_power4_presets(int cputype) {	
...
int _papi_hwd_init_global(void) 
{
   ...
   /* Setup presets */
   retval = setup_power4_presets(info.cpu_type);
   if (retval)
      return (retval);
...
static long_long get_cycles(void) {
   long_long ret = 0;
   ret = rdtsc_long();
