...
_niagara2_dispatch_timer (int signal, siginfo_t * si, void *info)
{
  ...
  // 
  if (ESI == NULL || ESI->master != thread || ESI->ctl_state == NULL || ((ESI->state & PAPI_OVERFLOWING) == 0))
    {
#ifdef DEBUG
      ...
      if(((ESI->state & PAPI_OVERFLOWING) == 0))
          SUBDBG (" -> %s: +++ Overflow flag missing, ESI->overflow.flags=%x\n", 
  ...
  if (_niagara2_read (ctrl, ctrl, (long_long **) & results, NOT_A_PAPI_HWD_READ)
      != PAPI_OK)
    {
      /* Failure */
...
_niagara2_set_overflow (EventSetInfo_t * ESI, int EventIndex, int threshold)
{
  ...
  SUBDBG (" -> %s: ESI->overflow.flags=%#x\n\n", __func__, ctrl, ESI->overflow.flags);
#endif
...
_niagara2_update_shlib_info (void)
{
  ...
  while (read (fd, &mapping, sizeof (prmap_t)) > 0)
	...
	{
#ifdef DEBUG
      SUBDBG (" -> %s: Anonymous mapping (MA_ANON) found for %s, skipping\n", 
                __func__, mapping.pr_mapname);
#endif          
	  continue;
	}
...
#ifdef DEBUG
      SUBDBG (" -> %s: Resolved name for %s: %s\n", __func__, mapping.pr_mapname, cur->name);
#endif
	}
	  ...
	  if (mapping.pr_mflags & MA_WRITE)
	    {
	      ...
	      cur->data_end = (caddr_t) (mapping.pr_vaddr + mapping.pr_size);
              ...
              if (strcmp(cur->name,_papi_hwi_system_info.exe_info.fullname) == 0)
	      {
		_papi_hwi_system_info.exe_info.address_info.data_start = cur->data_start;
		_papi_hwi_system_info.exe_info.address_info.data_end = cur->data_end;		
	      }              
	  ...
	  else if (mapping.pr_mflags & MA_EXEC)
	    {
	      ...
	      cur->text_end = (caddr_t) (mapping.pr_vaddr + mapping.pr_size);
              ...
              if (strcmp(cur->name,_papi_hwi_system_info.exe_info.fullname) == 0)
              {
                _papi_hwi_system_info.exe_info.address_info.text_start = cur->text_start;
		_papi_hwi_system_info.exe_info.address_info.text_end = cur->text_end;		
              }
...
/************* SUBSTRATE INTERFACE FUNCTIONS **********************************/  
  ...
  .bpt_map_exclusive        = NULL,                        /* NOT IMPLEMENTED */
  .bpt_map_shared           = NULL,                        /* NOT IMPLEMENTED */
  .bpt_map_preempt          = NULL,                        /* NOT IMPLEMENTED */
  .bpt_map_update           = NULL,                        /* NOT IMPLEMENTED */
  .allocate_registers       = NULL,                        /* NOT IMPLEMENTED */
