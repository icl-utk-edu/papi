...
static int
_any_shutdown( hwd_context_t * ctx )
{
	return ( PAPI_OK );
}

static int
_any_shutdown_substrate( void )
{
	return ( PAPI_OK );
}
 ...
 */
static int
_any_init_control_state( hwd_control_state_t * ptr )
{
	return PAPI_OK;
}

static int
_any_update_control_state( hwd_control_state_t * ptr, NativeInfo_t * native,
...
static int
_any_start( hwd_context_t * ctx, hwd_control_state_t * ctrl )
{
	return ( PAPI_OK );
}

static int
_any_read( hwd_context_t * ctx, hwd_control_state_t * ctrl, long long **events,
		   int flags )
{
	return ( PAPI_OK );
}
...
static int
_any_stop( hwd_context_t * ctx, hwd_control_state_t * ctrl )
{
	return ( PAPI_OK );
}

static int
_any_reset( hwd_context_t * ctx, hwd_control_state_t * ctrl )
{
	return ( PAPI_OK );
}

static int
_any_write( hwd_context_t * ctx, hwd_control_state_t * ctrl, long long *from )
{
	return ( PAPI_OK );
}
 ...
 */
static void
_any_dispatch_timer( int signal, hwd_siginfo_t * si, void *context )
{
...
static int
_any_stop_profiling( ThreadInfo_t * master, EventSetInfo_t * ESI )
{
	return ( PAPI_OK );
}

static int
_any_set_overflow( EventSetInfo_t * ESI, int EventIndex, int threshold )
{
	return ( PAPI_OK );
}

static int
_any_set_profile( EventSetInfo_t * ESI, int EventIndex, int threashold )
{
	return ( PAPI_OK );
}
 ...
 */
static int
_any_ctl( hwd_context_t * ctx, int code, _papi_int_option_t * option )
{
	return ( PAPI_OK );
}
 ...
 */
static int
_any_set_domain( hwd_control_state_t * cntrl, int domain )
{
	int found = 0;
...
_any_get_virt_usec( const hwd_context_t * ctx )
{
...
#if defined(USE_PROC_PTTIMER)
	{
		...
		retval =
			( utime +
			  stime ) * ( long long ) ( 1000000 / sysconf( _SC_CLK_TCK ) );
	}
 ...
 */
static int
_any_add_prog_event( hwd_control_state_t * ctrl, unsigned int EventCode,
					 void *inout, EventInfo_t * EventInfoArray )
{
	return ( PAPI_OK );
}

static int
_any_ntv_enum_events( unsigned int *EventCode, int modifier )
{
...
static int
_any_ntv_name_to_code( char *name, unsigned int *event_code )
{
...
static int
_any_ntv_code_to_name( unsigned int EventCode, char *name, int len )
{
...
static int
_any_ntv_code_to_descr( unsigned int EventCode, char *descr, int len )
{
	strncpy( descr,
			 "Event doesn't exist, is an example for a skeleton substrate",
			 len );
	return ( PAPI_OK );
}

static int
_any_ntv_code_to_bits( unsigned int EventCode, hwd_register_t * bits )
{
	return ( PAPI_OK );
}

static int
_any_ntv_bits_to_info( hwd_register_t * bits, char *names, unsigned int *values,
...
static int
_any_allocate_registers( EventSetInfo_t * ESI )
{
	return ( 1 );
}
...
static void
_any_bpt_map_set( hwd_reg_alloc_t * dst, int ctr )
{
}
 ...
 */
static int
_any_bpt_map_avail( hwd_reg_alloc_t * dst, int ctr )
{
	return ( 1 );
}
 ...
 */
static int
_any_bpt_map_exclusive( hwd_reg_alloc_t * dst )
{
	return ( 1 );
}
 ...
 */
static int
_any_bpt_map_shared( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	return ( 0 );
}
 ...
 */
static void
_any_bpt_map_preempt( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	return;
}

static void
_any_bpt_map_update( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	return;
}
...
_linux_update_shlib_info( void )
{
	...
	while ( 1 ) {
		if ( fgets( buf, sizeof ( buf ), f ) == NULL ) {
			if ( ferror( f ) ) {
				PAPIERROR( "fgets(%s, %d) returned < 0", fname,
						   sizeof ( buf ) );
				fclose( f );
				return ( PAPI_OK );
			} else
		...
		sscanf( buf, "%lx-%lx %4s %lx %5s %ld %s", &begin, &end, perm, &foo,
				dev, &inode, mapname );

		if ( strlen( mapname ) && ( perm[0] == 'r' ) && ( perm[1] != 'w' ) &&
			 ( perm[2] == 'x' ) && ( inode != 0 ) ) {
			upper_bound++;
		}
	}
	if ( upper_bound == 0 ) {
		PAPIERROR
			( "No segments found with r-x, inode != 0 and non-NULL mapname" );
		fclose( f );
		return ( PAPI_OK );
	}
	...
	tmp =
		( PAPI_address_map_t * ) papi_calloc( upper_bound,
											  sizeof ( PAPI_address_map_t ) );
	if ( tmp == NULL ) {
		PAPIERROR( "calloc(%d) failed",
				   upper_bound * sizeof ( PAPI_address_map_t ) );
		fclose( f );
		return ( PAPI_OK );
	}
	...
	while ( 1 ) {
		...
		if ( ( strcmp( find_data_mapname, mapname ) == 0 ) && ( perm[0] == 'r' )
			 && ( perm[1] == 'w' ) && ( inode != 0 ) ) {
	...
	for ( i = 0; i < count; i++ ) {
		if ( strcmp( tmp[i].name, _papi_hwi_system_info.exe_info.fullname ) ==
			 0 ) {
