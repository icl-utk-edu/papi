  ...
  *num = tmp;
  return(PAPI_OK);
   ...
   inline int pfmw_destroy_context(hwd_context_t *thr_ctx) {
      ...
      ret=perfmonctl(((ia64_context_t *)thr_ctx)->tid, PFM_DESTROY_CONTEXT, NULL, 0);
      if (ret) {   
...
#ifdef ITANIUM2
	int  r;
	int pid=((ia64_context_t *)pctx)->tid;
    pfmw_ita_param_t *param = &(((ia64_control_state_t *)current_state)->ita_lib_param);
    pfmw_param_t *evt=&(((ia64_control_state_t *)current_state)->evt);
...
*/
   ...
   inline int set_pmds_to_write(EventSetInfo_t * ESI, int index, int value)
   {
      ...
      pfmw_param_t *pevt= &(((ia64_control_state_t *)this_state)->evt);
      ...
      while (pos[count] != -1 && count < MAX_COUNTERS) {
         ...
         for (i = 0; i < MAX_COUNTERS; i++) {
            if ( PFMW_PEVT_PFPPC_REG_NUM(pevt,i) == hwcntr) {
               ((ia64_control_state_t *)this_state)->evt.pc[i].reg_smpl_pmds[0] = value;
               break;
            }
         }
         count++;
...
#if defined(ITANIUM3)
    pfmw_param_t *evt = &(((ia64_control_state_t *)current_state)->evt);
	pfmlib_mont_output_param_t *param = &(((ia64_control_state_t *)current_state)->ita_lib_param.mont_output_param);
	unsigned long umasks_retired[4];
	unsigned long umask;
...
#elif defined(ITANIUM2)
    pfmw_param_t *evt = &(((ia64_control_state_t *)current_state)->evt);
	pfmlib_ita2_output_param_t *param = &(((ia64_control_state_t *)current_state)->ita_lib_param.ita2_output_param);
	unsigned long umasks_retired[4];
	unsigned long umask;
	unsigned int j, i, seen_retired, ibrp, idx;
	int code;
...
{
#if defined(ITANIUM3)
	pfmlib_mont_output_param_t *param = (pfmlib_mont_output_param_t	*)&(((ia64_control_state_t *)current_state)->ita_lib_param.mont_output_param);
	...
	return (PAPI_OK);
#elif defined(ITANIUM2)
	pfmlib_ita2_output_param_t *param = (pfmlib_ita2_output_param_t *)&(((ia64_control_state_t *)current_state)->ita_lib_param.ita2_output_param);
