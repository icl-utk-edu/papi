...
*/
   ...
   inline int pfmw_create_ctx_common(hwd_context_t *ctx) 
   {
      ...
      SUBDBG("PFM_LOAD_CONTEXT FD %d, PID %d\n",((ia64_context_t *)ctx)->fd,((ia64_context_t *)ctx)->tid);
      if (perfmonctl(((ia64_context_t *)ctx)->fd, PFM_LOAD_CONTEXT, &load_args, 1) == -1) {
       ...
       */
      ret = fcntl(((ia64_context_t *)ctx)->fd, F_SETFL, fcntl(((ia64_context_t *)ctx)->fd, F_GETFL, 0) | O_ASYNC);
      if (ret == -1) {
         PAPIERROR("fcntl(%d,F_SETFL,O_ASYNC) errno %d", ((ia64_context_t *)ctx)->fd, errno);
         return(PAPI_ESYS);
      }
      ...
      ret = fcntl(((ia64_context_t *)ctx)->fd, F_SETOWN, ((ia64_context_t *)ctx)->tid);
      if (ret == -1) {
         PAPIERROR("fcntl(%d,F_SETOWN) errno %d", ((ia64_context_t *)ctx)->fd, errno);
         return(PAPI_ESYS);
      }
      ...
      if (ret == -1) {
	PAPIERROR("fcntl(%d,FD_CLOEXEC) errno %d", ((ia64_context_t *)ctx)->fd, errno);
	return(PAPI_ESYS);
      }
   ...
   inline int set_pmds_to_write(EventSetInfo_t * ESI, int index, unsigned long value)
   {
      int *pos, count, hwcntr, i;
      ia64_control_state_t *this_state = (ia64_control_state_t *)ESI->ctl_state;
...
static void check_ibrp_events(hwd_control_state_t *current_state)
{
	...
	case PFMLIB_ITANIUM2_PMU:
		...
		/*
			...
			*/
		for (i=0; i < ita2_output_param->pfp_ita2_irange.rr_nbr_used; i+= incr) {
	...
	case PFMLIB_MONTECITO_PMU:
		...
		/*
			...
			* This could otherwise lead to misinterpretation of the results.
			*/
		for (i=0; i < mont_output_param->pfp_mont_irange.rr_nbr_used; i+= incr) {
...
inline int install_irange(hwd_context_t *pctx, hwd_control_state_t *current_state)
{
	...
	case PFMLIB_ITANIUM2_PMU:
		...
		for(i=0; i < used_dbr; i++) {
			ita2_dbreg[i].dbreg_num   = ita2_output_param->pfp_ita2_irange.rr_br[i].reg_num; 
			ita2_dbreg[i].dbreg_value = ita2_output_param->pfp_ita2_irange.rr_br[i].reg_value; 
		}

		r = perfmonctl(pid, PFM_WRITE_IBRS, ita2_dbreg, ita2_output_param->pfp_ita2_irange.rr_nbr_used);
		if (r == -1){
	...
	case PFMLIB_MONTECITO_PMU:
		...
		for(i=0; i < used_dbr; i++) {
			mont_dbreg[i].dbreg_num   = mont_output_param->pfp_mont_irange.rr_br[i].reg_num; 
			mont_dbreg[i].dbreg_value = mont_output_param->pfp_mont_irange.rr_br[i].reg_value; 
		}

		r = perfmonctl(pid, PFM_WRITE_IBRS, mont_dbreg, mont_output_param->pfp_mont_irange.rr_nbr_used);
		if (r == -1){
			SUBDBG("cannot install code range restriction: %s\n", strerror(errno));
			return (PAPI_ESYS);
		}
...
inline int install_drange(hwd_context_t *pctx, hwd_control_state_t *current_state)
{
	...
	case PFMLIB_ITANIUM2_PMU:
		...
		for(i=0; i < used_dbr; i++) {
			ita2_dbreg[i].dbreg_num   = ita2_output_param->pfp_ita2_drange.rr_br[i].reg_num; 
			ita2_dbreg[i].dbreg_value = ita2_output_param->pfp_ita2_drange.rr_br[i].reg_value; 
		}

		r = perfmonctl(pid, PFM_WRITE_DBRS, ita2_dbreg, ita2_output_param->pfp_ita2_drange.rr_nbr_used);
		if (r == -1){
	...
	case PFMLIB_MONTECITO_PMU:
		...
		for(i=0; i < used_dbr; i++) {
			mont_dbreg[i].dbreg_num   = mont_output_param->pfp_mont_drange.rr_br[i].reg_num; 
			mont_dbreg[i].dbreg_value = mont_output_param->pfp_mont_drange.rr_br[i].reg_value; 
		}

		r = perfmonctl(pid, PFM_WRITE_DBRS, mont_dbreg, mont_output_param->pfp_mont_drange.rr_nbr_used);
		if (r == -1){
			SUBDBG("cannot install data range restriction: %s\n", strerror(errno));
			return (PAPI_ESYS);
		}
