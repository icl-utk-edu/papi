...
_papi_hwi_cleanup_all_presets( void )
{
    ...
    // TODO: Free _papi_hwi_comp_presets, too.
    for(cidx=0;cidx<papi_num_components;cidx++) {
      for ( preset_index = 0; preset_index < PAPI_MAX_PRESET_EVENTS; preset_index++ ) {
        /* Free the postfix. */
        if ( _papi_hwi_comp_presets[cidx][preset_index].postfix != NULL ) {
           papi_free( _papi_hwi_comp_presets[cidx][preset_index].postfix );
           _papi_hwi_comp_presets[cidx][preset_index].postfix = NULL;
        }
        /* Free the note. */
        if ( _papi_hwi_comp_presets[cidx][preset_index].note != NULL ) {
           papi_free( _papi_hwi_comp_presets[cidx][preset_index].note );
           _papi_hwi_comp_presets[cidx][preset_index].note = NULL;
        }
        /* Free the event names used to define the preset. */
        for(j=0; j<_papi_hwi_comp_presets[cidx][preset_index].count;j++) {
           papi_free(_papi_hwi_comp_presets[cidx][preset_index].name[j]);
           papi_free(_papi_hwi_comp_presets[cidx][preset_index].base_name[j]);
           papi_free(_papi_hwi_comp_presets[cidx][preset_index].default_name[j]);
        }
        /* Free the qualifier names and descriptions. */
        for(j=0; j<_papi_hwi_comp_presets[cidx][preset_index].num_quals;j++) {
           papi_free(_papi_hwi_comp_presets[cidx][preset_index].quals[j]);
           papi_free(_papi_hwi_comp_presets[cidx][preset_index].descrs[j]);
        }
      }
    }
	...
	*tmpn = '\0';
	...
	while (get_event_line(line, event_file, &event_table_ptr)) {
		...
		if (strcasecmp(t, comp_str) == 0) {
			...
			SUBDBG( "Use event code: %#x for %s\n", preset, t);
            unsigned int preset_index = ( preset & PAPI_PRESET_AND_MASK );
	        _papi_hwi_comp_presets[cidx][preset_index].compIdx = cidx;
			...
			do {
                ...
                if( firstTerm ) {
                    ...
                    do {
                        ...
                        const char *wholeNameConst = _papi_hwi_strip_component_prefix(eventInfo.symbol);
                        char *wholeName = strdup(wholeNameConst);
                        char *qualDelim = ":";
                        char *qualPtr = strstr( wholeName, qualDelim );
                        if( NULL != qualPtr ) {
                            ...
                            if( NULL != prstPtr->quals[prstPtr->num_quals-1] ) {
                                status = sprintf(prstPtr->quals[prstPtr->num_quals-1], "%s", qualPtr);
                                if( strlen(qualPtr) != status ) {
					                invalid_event = 1;
                            ...
                            /* If there is no qualifier detected, then even if 'masks:' exists in the event
                             * description, we don't want to track a 'qualifier description' because there
                             * is no associated qualifier. */
                            continue;
                        }

                        qualPtr = strstr( eventInfo.long_descr, "masks:" );
                        if( NULL != qualPtr ) {

                            qualPtr = strstr( qualPtr, ":" );
                            if( NULL != qualPtr && NULL != ++qualPtr) {
                    ...
                    } while( PAPI_OK == PAPI_enum_event( &eventCode, PAPI_NTV_ENUM_UMASKS) );

                    firstTerm = 0;
                }

                /* Construct event with all qualifiers. */
                int k, strLenSum = strlen(t)+1;
                ...
                if( NULL != tmpEvent ) {
                  ...
                  /* Print the basename to the string. */ 
                  status = sprintf(tmpEvent, "%s", t);
                  if( status != strlen(t) ) {
                      invalid_event = 1;
					  PAPIERROR("Event basename %s was truncated to %s in dervied event %s",
                                t, tmpEvent, results[res_idx].symbol);
					  break;
                  }

                  /* Print the qualifiers to the string. */
                  status = 0;
                  for (k = 0; k < prstPtr->num_quals; k++){
                      status += sprintf(tmpEvent, "%s%s", t, prstPtr->quals[k]);
                  }

                  if( status != strLenSum - 1 ) {
					  invalid_event = 1;
					  PAPIERROR("Event %s with qualifiers was truncated to %s in dervied event %s",
                                t, tmpEvent, results[res_idx].symbol);
					  break;
                  }
                }
