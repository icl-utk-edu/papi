    ...
    for(cidx=0;cidx<papi_num_components;cidx++) {
      for ( preset_index = 0; preset_index < _papi_hwi_max_presets[cidx]; preset_index++ ) {
        ...
        for(j=0; j<_papi_hwi_comp_presets[cidx][preset_index].num_quals;j++) {
           ...
           papi_free(_papi_hwi_comp_presets[cidx][preset_index].quals_descrs[j]);
        }
      }
    }
	...
	while (get_event_line(line, event_file, &event_table_ptr)) {
		...
		if (strcasecmp(t, comp_str) == 0) {
			...
			do {
                    ...
                    do {
                        ...
                        if( NULL != qualPtr ) {
                            ...
                            if( NULL != qualPtr && NULL != ++qualPtr) {
                                ...
                                size_t qualLen = 1+strlen(qualPtr);
                                prstPtr->quals_descrs[prstPtr->num_quals] = (char*)malloc(qualLen*sizeof(char));
                                if( NULL != prstPtr->quals_descrs[prstPtr->num_quals] ) {
                                    status = snprintf(prstPtr->quals_descrs[prstPtr->num_quals], qualLen, "%s", qualPtr);
                                    prstPtr->num_quals++;
                                    if( status >= qualLen ) {
					                    invalid_event = 1;
                ...
                int k, strLenSum = 0, baseLen = 1+strlen(t);
                ...
                if( NULL != tmpQuals || NULL != tmpEvent ) {
                  ...
                  status = snprintf(tmpEvent, baseLen, "%s", t);
                  if( status >= baseLen ) {
                      invalid_event = 1;
