		 ...
		 while ( ( feature = sensors_get_features ( chip_name, &a ) ) )
		 {
			 ...
			 while ( ( sub = sensors_get_all_subfeatures ( chip_name, feature, &b ) ) )
			 {
				 ...
				 strncpy ( lm_sensors_native_table[id].description,
						   lm_sensors_native_table[id].name,
						   PAPI_MAX_STR_LEN );
...
getEventValue ( unsigned event_id )
{
     ...
     res = sensors_get_value ( lm_sensors_native_table[event_id].resources.name,
							   lm_sensors_native_table[event_id].resources.subfeat_nr, 
							   &value );
...
LM_SENSORS_init_substrate (  )
{
     ...
     if ( ( lm_sensors_native_table = 
		    ( LM_SENSORS_native_event_entry_t * ) malloc ( sizeof ( LM_SENSORS_native_event_entry_t ) * NUM_EVENTS ) ) == NULL )
     {
...
LM_SENSORS_init_control_state ( hwd_control_state_t * ctrl )
{
     ...
     for ( i = 0; i < NUM_EVENTS; i++ )
		 ( ( LM_SENSORS_control_state_t * ) ctrl )->counts[i] = getEventValue ( i );
	
     ( ( LM_SENSORS_control_state_t * ) ctrl )->lastupdate = PAPI_get_real_usec (  );
     return PAPI_OK;
}
		  ...
		  long_long ** events, int flags )
{
     ...
     if ( start - ( ( LM_SENSORS_control_state_t * ) ctrl )->lastupdate > 200000 )
     {				// cache refresh
		 int             i;
				  ...
				  hwd_context_t * ctx )
{
     ...
     for ( i = 0; i < count; i++ )
     {
		 ...
		 native[i].ni_position = lm_sensors_native_table[index].resources.selector - 1;
     }
     return ( PAPI_OK );
}
