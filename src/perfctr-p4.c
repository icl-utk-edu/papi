...
extern int _p4_ntv_code_to_bits( unsigned int EventCode,
								 hwd_register_t * bits );
extern int _p4_ntv_bits_to_info( hwd_register_t * bits, char *names,
...
*/
static int
_p4_init_control_state( hwd_control_state_t * ptr )
{
	int def_mode = 0, i;
...
static int
_p4_start( hwd_context_t * ctx, hwd_control_state_t * state )
{
	int error;
#ifdef DEBUG
...
static int
_p4_stop( hwd_context_t * ctx, hwd_control_state_t * state )
{
	if ( state->rvperfctr != NULL ) {
...
static int
_p4_read( hwd_context_t * ctx, hwd_control_state_t * spc,
...
*/
static int
_p4_bpt_map_avail( hwd_reg_alloc_t * dst, int ctr )
{
...
*/
static void
_p4_bpt_map_set( hwd_reg_alloc_t * dst, int ctr )
{
...
*/
static int
_p4_bpt_map_exclusive( hwd_reg_alloc_t * dst )
{
...
*/
static int
_p4_bpt_map_shared( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	int retval1, retval2;
...
*/
static void
_p4_bpt_map_preempt( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
	int i;
	unsigned shared;
...
*/
static void
_p4_bpt_map_update( hwd_reg_alloc_t * dst, hwd_reg_alloc_t * src )
{
...
static int
_p4_allocate_registers( EventSetInfo_t * ESI )
{
	int i, j, natNum;
	...
	for ( i = 0; i < natNum; i++ ) {
		...
		_papi_pfm_ntv_code_to_bits( ESI->NativeInfoArray[i].ni_event,
									&e->ra_bits );
...
static int
_p4_update_control_state( hwd_control_state_t * this_state,
...
static int
_p4_set_domain( hwd_control_state_t * cntrl, int domain )
{
	int i, did = 0;
...
static int
_p4_reset( hwd_context_t * ctx, hwd_control_state_t * cntrl )
{
...
static void
swap_events( EventSetInfo_t * ESI, struct vperfctr_control *contr, int cntr1,
			 int cntr2 )
{
	unsigned int ui;
	int si, i, j;
...
static int
_p4_set_overflow( EventSetInfo_t * ESI, int EventIndex, int threshold )
{
	...
	if ( threshold != 0 ) {	 /* Set an overflow threshold */
		...
		if ( ( retval =
			   _papi_hwi_start_signal( MY_VECTOR.cmp_info.hardware_intr_sig,
									   NEED_CONTEXT,
