...
extern int _aix_update_control_state(hwd_control_state_t *this_state, NativeInfo_t *native, int count, hwd_context_t *context);
extern int _aix_ntv_bits_to_info(hwd_register_t *bits, char *names, unsigned int *values, int name_len, int count);
...
int
_aix_ntv_code_to_descr ( unsigned int EventCode, char *ntv_descr,
			      int len )
{
     if ( ( EventCode & PAPI_NATIVE_AND_MASK ) >=
	  MY_VECTOR.cmp_info.num_native_events )
...
round_requested_ns ( int ns )
{
     ...
     {
	  int             leftover_ns =
	       ns % _aix_vector.cmp_info.itimer_res_ns;
...
*/
     case PAPI_DOMAIN:
	  return(_aix_set_domain(option->domain.ESI->ctl_state, option->domain.domain));
...
_aix_dispatch_timer ( int signal, siginfo_t * si, void *i )
{
     ...
     _papi_hwi_dispatch_overflow_signal ((void *) &ctx, address, NULL, 0, 0, &t , MY_VECTOR.cmp_info.CmpIdx);
}
