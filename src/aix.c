...
int
_papi_hwd_ntv_code_to_descr ( unsigned int EventCode, char *ntv_descr,
			      int len )
{
     if ( ( EventCode & PAPI_NATIVE_AND_MASK ) >=
	  _papi_hwi_system_info.sub_info.num_native_events )
...
_papi_hwd_ntv_enum_events ( unsigned int *EventCode, int modifier )
{
     ...
     else if ( modifier == PAPI_NTV_ENUM_GROUPS )
     {
	  ...
	  unsigned int    group =
	       ( *EventCode & PAPI_NTV_GROUP_AND_MASK ) >>
	  ...
	  *EventCode = *EventCode & ( ~PAPI_NTV_GROUP_SHIFT );
	  for ( i = 0; i < GROUP_INTS; i++ )
	  {
	       ...
	       if ( tmpg != 0 )
	       {
		    group = ffs ( tmpg ) + i * 32;
		    *EventCode =
			 *EventCode | ( group << PAPI_NTV_GROUP_SHIFT );
		    return ( PAPI_OK );
	       }
	  }
#endif
...
long long
_papi_hwd_get_virt_usec ( const hwd_context_t * context )
{
     long long       retval;
     struct tms      buffer;
...
long long
_papi_hwd_get_virt_cycles ( const hwd_context_t * context )
{
     return ( _papi_hwd_get_virt_usec ( context ) *
	      ( long long ) _papi_hwi_system_info.hw_info.mhz );
}

static void
_papi_lock_init ( void )
{
     int             i;
 ...
 */
     ...
     if ( ( retval = power3_setup_vector_table ( vtable ) ) != 0 )
	  return retval;
#else
     _papi_hwi_system_info.sub_info.num_native_events =
	  ppc64_setup_native_table ( vtable );
     if ( ( retval = ppc64_setup_vector_table ( vtable ) ) != 0 )
	  return retval;
#endif
...
int
_papi_hwd_init ( hwd_context_t * context )
{
     int             retval;
...
int
_papi_hwd_reset ( hwd_context_t * ESI, hwd_control_state_t * zero )
{
...
int
_papi_hwd_read ( hwd_context_t * ctx, hwd_control_state_t * spc,
...
round_requested_ns ( int ns )
{
     ...
     {
	  int             leftover_ns =
	       ns % _papi_hwi_system_info.sub_info.itimer_res_ns;
...
*/
     ...
     case PAPI_DEF_ITIMER:
	  {
	       ...
	       if ( option->itimer.ns > 0 )
		    option->itimer.ns =
			 round_requested_ns ( option->itimer.ns );
     ...
     case PAPI_DEF_MPX_NS:
	  {
	       option->multiplex.ns =
		    round_requested_ns ( option->multiplex.ns );
	       return ( PAPI_OK );
	  }
...
void
_papi_hwd_dispatch_timer ( int signal, siginfo_t * si, void *i )
{
...
int
_papi_hwd_start ( hwd_context_t * ctx, hwd_control_state_t * cntrl )
{
     int             i, retval;
     ...
     if ( retval > 0 )
     {
	  if ( retval == 13 )
	  {
	       ...
	       retval =
		    pm_set_program_mythread ( &current_state->counter_cmd );
	       if ( retval > 0 )
	       {
		    if ( _papi_hwi_error_level != PAPI_QUIET )
			 pm_error ( "PAPI Error: pm_set_program_mythread",
				    retval );
		    return ( retval );
	       }
	  }
...
int
_papi_hwd_stop ( hwd_context_t * ctx, hwd_control_state_t * cntrl )
{
     int             retval;
...
_papi_hwd_update_shlib_info ( void )
{
     ...
     while ( ( retval = fread ( &newp, sizeof ( prmap_t ), 1, map_f ) ) > 0 )
     {
	  if ( newp.pr_pathoff > 0 && newp.pr_mapname[0] != '\0' )
	  {
	       ...
	       if ( ( newp.pr_mflags & MA_READ ) &&
		    ( newp.pr_mflags & MA_EXEC ) )
		    t_index++;
	  }
     }
     rewind ( map_f );
     ...
     t_index = -1;
     while ( ( retval =
	       fread ( &tmp1[i], sizeof ( prmap_t ), 1, map_f ) ) > 0 )
     {
     ...
     for ( i = 0; i < count; i++ )
     {
	  ...
	  {
	       if ( strlen( _papi_hwi_system_info.exe_info.address_info.name ) != cc - 1 )
		    PAPIERROR( "executable name too long (%d char). Match of first %d char only",
			   strlen ( _papi_hwi_system_info.exe_info.address_info.name ), cc - 1 );
	  ...
	  else
	  {
	       if ( ( _papi_hwi_system_info.exe_info.address_info.text_start == 0 ) &&
		    ( _papi_hwi_system_info.exe_info.address_info.text_end == 0 ) &&
		    ( _papi_hwi_system_info.exe_info.address_info.data_start == 0 ) &&
		    ( _papi_hwi_system_info.exe_info.address_info.data_end == 0 ) )
	       ...
	       if ( tmp1[i].pr_mflags & MA_READ )
	       {
		    ...
		    {
			 t_index++;
			 tmp2[t_index].text_start =
			      ( caddr_t ) tmp1[i].pr_vaddr;
			 tmp2[t_index].text_end =
			      ( caddr_t ) ( tmp1[i].pr_vaddr +
					    tmp1[i].pr_size );
			 strncpy ( tmp2[t_index].name, name,
				   PAPI_MAX_STR_LEN );
		    }
		    else if ( tmp1[i].pr_mflags & MA_WRITE )
		    {
			 tmp2[t_index].data_start =
			      ( caddr_t ) tmp1[i].pr_vaddr;
