 ...
 *
 * In this version the extrapolation is made in a local time scale. At
...
/* More info
In this version the counts recorded during a time slice is taken to be
the rate of this event since the event was sliced out the previous
time.  This is somewhat in contrast to the current sources where the
total acquried counts over the complete measurement time since
PAPI_start over the total time the event has been active is taken as
the rate for the complete time span.

In mathematical terms we can compare these two counts as follows:

Let c1, c2, c3, .., cn be the counts registered each time slice a
specific counter has been active. Let t1, t2, t3, ..., tn be the time
count the counter was active each time. Let T1, T2, ..., Tn be the
time between the end of each time slice. I.e.
     T1 = time when first time slice was completed for the event minus
          the time of PAPI_start
     T2 = time when second time slice was completed for the event minus
          time when first time slice was completed for the event
     T3 = time when third time slice was completed for the event minus
          time when second time slice was completed for the event
     ...

The original source code derived the achieved count by the formula:

  Count = (c1+c2+...+cn) * (T1+T2+...+Tn) / (t1+t2+...+tn)

This modified source code derives the count as:

  Count = c1*T1/t1 + c2*T2/t2 + ... + cn*Tn/tn

In the case where the time slices are very exact (so that tx/Tx is
constant) the two counts will be equal. However, any PAPI_read is
likely to occur at a point in time such that Tn/tn will not be the
same as the other ratios. Also, depending on the operating system it
is likely that the ratios varies even at other times.  The latter
variation is due to that the timer interrupt is not too precise.

If the counts also varies heavily, for example due to that come counts
c1, c2, ..., ck occur in a computational intensive section of code,
while the later ck,...cn counts occur in less intensive parts of the
code, the effect of the varying tx/Tx ratios can be amplified or
diminished.

The counts returned by PAPI_read calls will be the difference between
the estimated current counter values and the estimated counter values
at the latest previous PAPI_start or PAPI_reset. */
  ...
  if( threads_responding == 0 ) {       /* this thread caught the timer sig */
    ...
    for( t = tlist; t != NULL; t = t->next ) {
      if( pthread_equal(t->thr, self) == 0 ) {
        ++threads_responding;
        retval = pthread_kill(t->thr, SIGVTALRM);
        assert(retval == 0);
#ifdef MPX_DEBUG_SIGNALS
    ...
    if( me != NULL && me->cur_event != NULL ) {
      ...
      if( retval == PAPI_OK ) {
        ...
        if ( cur_event->is_a_rate ) {
          ...
          if( cycles > 1000 ) /* Only update current rate on a decent slice */
            cur_event->rate_estimate = (double) counts[0] / (double) cycles ;
          cur_event->count_estimate += (long_long) (total_cycles * cur_event->rate_estimate);
        }
      } else {
...
#ifdef MPX_DEBUG
      fprintf(stderr, "%x value = %lld (%lld) cycles = %lld (%lld) rate = %lf\n\n",
...
#ifdef MPX_DEBUG_TIMER
  else fprintf(stderr,"nothing to do in thread %x\n", me->pid);
#endif
...
int MPX_start(MPX_EventSet * mpx_events)
{
  ...
  if( t->cur_event && t->cur_event->active ) {
    ...
    assert(retval == PAPI_OK);
    cycles_this_slice =
      ((t->cur_event->pi.event_type == PAPI_TOT_CYC)
       ? values[0] : values[1]);
  } else {
    values[0] = values[1] = 0;
  }
  ...
  for( i = 0; i < mpx_events->num_events; i++ ) {
    ...
    long_long prev_count, prev_cycles, prev_hc;
...
#ifndef MPX_NONDECR
        if( mev == t->cur_event ) {
          /* Use previous rate if the event has run too short time */
          if( cycles_this_slice < 1000 )
...
#else
    /* Each event has its own "stop" time: mev->prev_total_c */
    mpx_events->stop_c = -1; 
#endif
...
int MPX_stop(MPX_EventSet * mpx_events, long_long * values)
{
  int i;
  int retval;
  long_long last_value[2], dummy_value[2];
  ...
  for( i = 0; i < mpx_events->num_events; i++ ) {
    ...
    if( mev == cur_event ) {
      ...
      if( mev->active == 0  ) {
        ...
        /* We'll discard the events that occured from PAPI_read
         * above until here
         */
        /*
         *cur_event_cycles
        ...
        thr->total_c += cycles_this_slice;
        total_cycles = thr->total_c - cur_event->prev_total_c;
        cur_event->prev_total_c = thr->total_c;
        cur_event->handler_count++;
        ...
        if( mev->is_a_rate ) {
          if( cycles_this_slice >= MINCYCLES ) {
            cur_event->cycles++;
          } else {
            cur_event->count += last_value[0];
          }
        } else {
          cur_event->cycles += cycles_this_slice;
          if( cycles_this_slice > 1000 ) 
            cur_event->rate_estimate = cycles_this_slice ?
              (double) last_value[0] / (double) cycles_this_slice : 0.0;
          cur_event->count_estimate += (long_long) (total_cycles * cur_event->rate_estimate);
        }
	...
	/*
        final_count = mev->count_estimate;
	...
	mpx_events->stop_values[i] = mev->count_estimate;
        if( !(mev->is_a_rate) ) {
          ...
          mpx_events->stop_values[i] += last_value[0] + 
            (long_long) (mev->rate_estimate * 
                         ( thr->total_c - mev->prev_total_c));
#endif
        } else {
          mpx_events->stop_values[i] = mev->count;
          mpx_events->stop_hc[i] = mev->cycles;
        }
      }
    } else {
      if( !(mev->is_a_rate) ) {
	...
	mpx_events->stop_values[i] += 
          (long_long) (mev->rate_estimate * 
                       ( cycles_this_slice + thr->total_c - mev->prev_total_c ));
#endif
      } else {
	mpx_events->stop_values[i] = mev->count;
	mpx_events->stop_hc[i] = mev->cycles;
