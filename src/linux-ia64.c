...
static preset_search_t preset_search_map[] = { 
  ...
  {PAPI_L2_ICM,DERIVED_SUB,{"L2_MISSES","L3_READS.INST_READS.ALL",0,0}},
  {PAPI_L2_DCM,0,{"L3_READS.INST_READS.ALL",0,0,0}},
...
inline static int set_domain(hwd_control_state_t *this_state, int domain)
{
  ...
  for (i=0;i<PMU_MAX_COUNTERS;i++)
	...
	{
	  perfmon_reg_t reg;
	  reg.pmu_reg = this_state->pc[i].pfr_reg.reg_value;
	  ...
	  reg.pmc_plm = mode;
	  this_state->pc[i].pfr_reg.reg_value = reg.pmu_reg;
