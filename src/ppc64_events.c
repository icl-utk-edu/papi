...
void ppc64_setup_gps(int total, ntv_event_group_info_t * group_info)
{
	int i, j, gnum;
			...
			if (native_table[i].resources.selector & (1 << j)) {
				for (gnum = 0; gnum < group_info->maxgroups; gnum++) {
					if (native_table[i].resources.counter_cmd[j] ==
						...
						native_table[i].resources.group[gnum/32] |= 1 << (gnum % 32);
					}
				}
			}
		}
	...
	for (gnum = 0; gnum < group_info->maxgroups; gnum++) {
		...
		for (i = 0; i < MAX_COUNTERS; i++)
			group_map[gnum].counter_cmd[i] = group_info->event_groups[gnum]->events[i];
	}
}
	...
	for (pmc = 0; pmc < info->maxpmcs; pmc++) {
		...
		for (ev = 0; ev < info->maxevents[pmc]; ev++, wevp++ ) {      	 
			...
			if (i == index) {
				...
				native_table[i].resources.selector |= 1 << pmc;
				native_table[i].resources.counter_cmd[pmc] = wevp->event_num;
				native_table[i].name = (char *) malloc(strlen(wevp->symbol) +1);
...
static FILE * open_file(const char * fname) 
{
	...
	char * relative_pathname = (char *) malloc(strlen("/") + 
		strlen("event_data") + strlen("/") + strlen(cpu) + strlen("/") + 
		strlen(fname) + 1);
	...
	char * pathname = (char *) malloc(strlen(dir) + strlen(relative_pathname) + 4);
	int keep_trying;
	if (strcmp(dir, dot) == 0)	
		keep_trying = 2;
	else
...
static ntv_event_t * copy_buffer(ntv_event_t events[], int maxevents ) {
	...
	for (cnt = 0; cnt < maxevents; cnt++, cur_wev++) {
		...
		cur_wev->short_description = (char *) malloc(strlen(events[cnt].short_description));
		strcpy(cur_wev->short_description, events[cnt].short_description);
		cur_wev->description = (char *) malloc(strlen(events[cnt].description));
			...
			case 1:
				...
				for (i = cc = 0; i < len; i++) {
					...
					if ( cc == 5 && !symb_found) {
						...
						len = strlen(&data[i+1]);
						events[num_events].short_description = (char *) malloc(len);
						strcpy(events[num_events].short_description, &data[i+1]);
...
static ntv_event_group_info_t * parse_groupfile(FILE * grp_file) 
{
	...
	ntv_event_group_info_t * group_info = 
		(ntv_event_group_info_t *) malloc(sizeof(ntv_event_group_info_t));
	...
	while ( fgets(data, 1022, grp_file) ) {
		...
		switch (state) {
			case 0:				
...
#else
				if ( sscanf(data, "#%u,%u,%u,%u,%u,%u,%u,%u,%u", &g,&event_num[0],&event_num[1],&event_num[2],&event_num[3],&event_num[4],&event_num[5],&event_num[6],&event_num[7]) == 9 ) {
					state = 1;
				...
				if (state ==1) {
					...
					for (; j < MAX_COUNTERS; j++) 
						group_info->event_groups[group_info->maxgroups]->events[j] = event_num[j];
				}
				break;
			case 1:
			...
			case 2:
			/* get mmcr values */
				if ( sscanf(data, "%x,%x,%x,%x", &mmcr0, &mmcr1H, &mmcr1L, &mmcra) != 4 ) {
