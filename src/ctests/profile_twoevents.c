...
int main(int argc, char **argv) 
{
 ...
 if ((prginfo = PAPI_get_executable_info()) == NULL){
  ...
  if ((retval=PAPI_profil(profbuf4, length, start, 65536, 
	     EventSet, PAPI_TOT_CYC, THRESHOLD, PAPI_PROFIL_POSIX)) != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  ...
  if ((retval=PAPI_profil(profbuf5, length, start, 65536, 
	  EventSet, PAPI_TOT_CYC, 0, PAPI_PROFIL_POSIX | PAPI_PROFIL_RANDOM))
	 != PAPI_OK)
        test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  ...
  if ((retval=PAPI_profil(profbuf6, length, start, 65536, 
        EventSet, PAPI_TOT_CYC, THRESHOLD, PAPI_PROFIL_POSIX|PAPI_PROFIL_WEIGHTED))
	 != PAPI_OK)
        test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  ...
  if ((retval=PAPI_profil(profbuf3, length, start, 65536, 
	...
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  if ((retval=PAPI_profil(profbuf6, length, start, 65536, 
        EventSet, PAPI_TOT_CYC, 0, PAPI_PROFIL_POSIX|PAPI_PROFIL_WEIGHTED))
	 != PAPI_OK)
        test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
