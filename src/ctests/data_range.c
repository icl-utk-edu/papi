...
/***************************************************************************************/
   ...
   printf("These loads result from accessing the pointers to compute array addresses.\n");
...
static void measure_load_store(caddr_t start, caddr_t end) {
   ...
   if ((retval = PAPI_set_opt(PAPI_DATA_ADDRESS, &option)) != PAPI_OK)
      test_fail(__FILE__, __LINE__, "PAPI_set_opt(PAPI_DATA_ADDRESS)", retval);
...
*/	      
		printf("Requested Start Address: %p; Start Offset: 0x%5x; Actual Start Address: %p\n",
			option->addr.start, option->addr.start_off, option->addr.start - option->addr.start_off);
		printf("Requested End   Address: %p; End   Offset: 0x%5x; Actual End   Address: %p\n",
			option->addr.end, option->addr.end_off, option->addr.end + option->addr.end_off);
	}
   ...
   if ((retval = PAPI_remove_event(EventSet, PAPI_event[index])) != PAPI_OK)
