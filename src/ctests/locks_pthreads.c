...
void *Master(void *arg)
{
  ...
  int i;
  usleep(SLEEP_VALUE);
  PAPI_lock(PAPI_USR1_LCK); 
  /* Make sure Slaves are not sleeping */
  for(i=0;i<LOOPS;i++){
	count= 2*count/3
  }
  PAPI_unlock(PAPI_USR1_LCK);
  pthread_exit(NULL);
}
...
void *Slave(void *arg)
{
  ...
  PAPI_lock(PAPI_USR2_LCK);
  PAPI_lock(PAPI_USR1_LCK); 
  for(i=0;i<LOOPS;i++){
	count+=i;
  }
  PAPI_unlock(PAPI_USR1_LCK);
  PAPI_unlock(PAPI_USR2_LCK);
  pthread_exit(NULL);
}
...
main(int argc, char **argv )
{
  ...
  PAPI_lock(PAPI_USR2_LCK);
  rc = pthread_create(&master, NULL, Master, (void *) &result);
  if (rc){
        retval=PAPI_ESYS;
        test_fail(__FILE__,__LINE__,"pthread_create",retval);
  }
  rc = pthread_create(&slave1, NULL, Slave, (void *) &result);
  if (rc){
        retval=PAPI_ESYS;
        test_fail(__FILE__,__LINE__,"pthread_create",retval);
  }
