...
int main(int argc, char **argv) 
{
  ...
  if ( !TESTS_QUIET ) {
	...
	printf(TAB1, "Row 3 approximately equals",(values[0])[1]/(long_long)THRESHOLD);
  }
  ...
  min = (long_long)((values[0][1]*.80)/(long_long)THRESHOLD);
  max = (long_long)((values[0][1]*1.15)/(long_long)THRESHOLD);
