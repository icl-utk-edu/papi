...
int main(int argc, char **argv) 
{
  ...
  unsigned int *profbuf2;
  unsigned int *profbuf3;
  unsigned int *profbuf4;
  unsigned int *profbuf5;
 ...
 if ((prginfo = PAPI_get_executable_info()) == NULL){
  ...
  length = (end - start)/sizeof(int)*sizeof(int);

  profbuf = (unsigned int *)malloc(length);
  if (profbuf == NULL){
	retval=PAPI_ESYS;
	test_fail(__FILE__,__LINE__,"malloc",retval);
  }
  memset(profbuf,0x00,length);
  profbuf2 = (unsigned int *)malloc(length);
  if (profbuf2 == NULL){
	retval=PAPI_ESYS;
	test_fail(__FILE__,__LINE__,"malloc",retval);
  }
  memset(profbuf2,0x00,length);
  profbuf3 = (unsigned int *)malloc(length);
  if (profbuf3 == NULL){
	retval=PAPI_ESYS;
	test_fail(__FILE__,__LINE__,"malloc",retval);
  }
  memset(profbuf3,0x00,length);
  profbuf4 = (unsigned int *)malloc(length);
  if (profbuf4 == NULL){
	retval=PAPI_ESYS;
	test_fail(__FILE__,__LINE__,"malloc",retval);
  }
  memset(profbuf4,0x00,length);
  profbuf5 = (unsigned int *)malloc(length);
  if (profbuf5 == NULL){
	retval=PAPI_ESYS;
	test_fail(__FILE__,__LINE__,"malloc",retval);
  }
  ...
  if ( !TESTS_QUIET ) {
	...
	if ((retval=PAPI_profil(profbuf, length, start, 65536, 
		 EventSet, PAPI_event, THRESHOLD, PAPI_PROFIL_POSIX|PAPI_PROFIL_BUCKET_32)) != PAPI_OK){
  ...
  if ((retval=PAPI_profil(profbuf, length, start, 65536, 
	     EventSet, PAPI_event, 0, PAPI_PROFIL_POSIX|PAPI_PROFIL_BUCKET_32)) != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  ...
  if ((retval=PAPI_profil(profbuf2, length, start, 65536, 
		     ...
		     PAPI_PROFIL_POSIX | PAPI_PROFIL_RANDOM|PAPI_PROFIL_BUCKET_32)) != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  ...
  if ((retval=PAPI_profil(profbuf2, length, start, 65536, 
	  EventSet, PAPI_event, 0, PAPI_PROFIL_POSIX | PAPI_PROFIL_RANDOM|PAPI_PROFIL_BUCKET_32))
	 != PAPI_OK)
        test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  ...
  if ((retval=PAPI_profil(profbuf3, length, start, 65536, 
        EventSet, PAPI_event, THRESHOLD, PAPI_PROFIL_POSIX|PAPI_PROFIL_WEIGHTED|PAPI_PROFIL_BUCKET_32))
	 != PAPI_OK)
        test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  ...
  if ((retval=PAPI_profil(profbuf3, length, start, 65536, 
	EventSet, PAPI_event, 0, PAPI_PROFIL_POSIX | PAPI_PROFIL_WEIGHTED|PAPI_PROFIL_BUCKET_32))
	 != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  if ( !TESTS_QUIET )
      printf("Test type   : \tPAPI_PROFIL_COMPRESS\n");
  if ((retval=PAPI_profil(profbuf4, length, start, 65536, 
	EventSet, PAPI_event,THRESHOLD,PAPI_PROFIL_POSIX | PAPI_PROFIL_COMPRESS|PAPI_PROFIL_BUCKET_32))
	 != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  if ((retval=PAPI_start(EventSet)) != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_start",retval);
  ...
  if ((retval=PAPI_profil(profbuf4, length, start, 65536, 
	EventSet, PAPI_event, 0, PAPI_PROFIL_POSIX | PAPI_PROFIL_COMPRESS|PAPI_PROFIL_BUCKET_32))
	 != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  if ( !TESTS_QUIET )
  ...
  if ((retval=PAPI_profil(profbuf5, length, start, 65536, 
		     ...
		     PAPI_PROFIL_RANDOM |
		     PAPI_PROFIL_COMPRESS|PAPI_PROFIL_BUCKET_32)) != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
  if ((retval=PAPI_start(EventSet)) != PAPI_OK)
	test_fail(__FILE__,__LINE__,"PAPI_profil",retval);
