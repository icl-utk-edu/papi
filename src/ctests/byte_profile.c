...
static int do_profile(unsigned long plength, unsigned scale, int thresh, int bucket) {
   ...
   if (!TESTS_QUIET) {
      printf(TAB1, "PAPI_TOT_CYC:", (values[0])[3]);
      printf(TAB1, "PAPI_TOT_INS:", (values[0])[2]);
      printf(TAB1, "PAPI_FP_OPS:", (values[0])[1]);
      printf(TAB1, "PAPI_L1_DCM:", (values[0])[0]);
   }
   ...
   prof_head(blength, bucket, num_buckets, 
      "address\t\t\tcyc\tins\tfp_ops\tl1_dcm\n");
   prof_out(4, bucket, num_buckets, scale);
