/*
* File:    pfm4_perf_events.c
* CVS:     $Id$
* Author:   Dan Terpstra
*           terpstra@eecs.utk.edu
*          - based upon perf_events.c written by -
*           Corey Ashford
*           cjashfor@us.ibm.com
*/
...
volatile unsigned int _pfm4_lock_data[PAPI_MAX_LOCK];
extern papi_vector_t _pfm4_vector;

extern int _papi_pfm4_setup_presets( char *name, int type );
//extern int _papi_pfm4_ntv_enum_events( unsigned int *EventCode, int modifier );
//extern int _papi_pfm4_ntv_name_to_code( char *ntv_name,
//									   unsigned int *EventCode );
//extern int _papi_pfm4_ntv_code_to_name( unsigned int EventCode, char *ntv_name,
//									   int len );
//extern int _papi_pfm4_ntv_code_to_descr( unsigned int EventCode, char *ntv_descr,
//										int len );
...
//									   int count );
extern int _papi_pfm4_ntv_enum_named_events( char *EventName, int len, int modifier );
extern int _papi_pfm4_ntv_name_to_descr( char *EventName, char *ntv_descr,
										int len );
extern int _papi_pfm4_ntv_name_to_bits( char *EventName, hwd_register_t * bits );
extern int _papi_pfm4_ntv_name_to_info( char *EventName, char *names,
									   unsigned int *values, int name_len,
									   int count );
extern int get_cpu_info( PAPI_hw_info_t * hwinfo );

/* Forward function declarations */
int _pfm4_update_control_state( hwd_control_state_t * ctl,
								   NativeInfo_t * native, int count,
								   hwd_context_t * ctx );
int _pfm4_set_overflow( EventSetInfo_t * ESI, int EventIndex,
						   int threshold );
...
int
_pfm4_get_dmem_info( PAPI_dmem_info_t * d )
{
...
#if defined(PFM_PMU_MIPS_ICE9A)&&defined(PFM_PMU_MIPS_ICE9B)
		switch ( _pfm4_pmu_type ) {
		case PFM_PMU_MIPS_ICE9A:
		case PFM_PMU_MIPS_ICE9B:
...
sparc_get_memory_info( PAPI_hw_info_t * hw_info )
{
	...
	*t = '\0';
	...
	case PFM_PMU_SPARC_ULTRA3:
	case PFM_PMU_SPARC_ULTRA3I:
	case PFM_PMU_SPARC_ULTRA3PLUS:
	case PFM_PMU_SPARC_ULTRA4PLUS:
...
int
_pfm4_get_memory_info( PAPI_hw_info_t * hwinfo, int unused )
{
	( void ) unused;		 /*unused */
	int retval = PAPI_OK;
...
get_system_info( papi_mdi_t * mdi )
{
	...
	retval = _pfm4_update_shlib_info(  );
	...
	retval = _pfm4_get_memory_info( &mdi->hw_info, mdi->hw_info.model );
	if ( retval )
		return retval;
...
_pfm4_init_cmp( int cidx )
{
	...
	SUBDBG( "pfm_get_version() returns %d.%d\n", PFM_MAJ_VERSION( version ),
	...
	pmu_name[0] = '\0';
	/* Walk the entire list of pmus. More than one might
		be valid for a given component. For instance,
		Nehalem and Nehalem Uncore. For now, always pick the first one.
		XXXXX What to do about multiples??
	*/
	for(i=0; i < PFM_PMU_MAX; i++) {
		...
		if (pinfo.is_present) {
			...
			strncpy( pmu_name, pinfo.name, PAPI_MIN_STR_LEN );
			_pfm4_pmu_type = pinfo.pmu;
			break;
		}
	}
	if( !pmu_name[0] ) {
		PAPIERROR( "PMU not found\n" );
		return PAPI_ESBSTR;
	}
	...
	if ( ( ncnt = pfm_get_nevents(  ) ) == 0 ) {
		PAPIERROR( "pfm_get_nevents(): %s\n",
				   pfm_strerror( retval ) );
		return PAPI_ESBSTR;
	}
	SUBDBG( "pfm_get_nevents: %d\n", ncnt );
	...
	/* Setup presets */
	retval = _papi_pfm4_setup_presets( pmu_name, _pfm4_pmu_type );
	if ( retval )
		return retval;
...
long long
_pfm4_get_virt_usec( const hwd_context_t * zero )
{
...
long long
_pfm4_get_virt_cycles( const hwd_context_t * zero )
{
	return _pfm4_get_virt_usec( zero ) *
		( long long ) _papi_hwi_system_info.hw_info.mhz;
}
...
int _pfm4_stop( hwd_context_t * ctx, hwd_control_state_t * ctl );

/* reset the hardware counters */
int
_pfm4_reset( hwd_context_t * ctx, hwd_control_state_t * ctl )
{
	int i, ret;
...
int
_pfm4_write( hwd_context_t * ctx, hwd_control_state_t * ctl,
				long long *from )
{
	( void ) ctx;			 /*unused */
	( void ) ctl;			 /*unused */
...
int
_pfm4_read( hwd_context_t * ctx, hwd_control_state_t * ctl,
...
#if defined(__crayxt)
int _pfm4_start_create_context = 0; /* CrayPat checkpoint support */
#endif /* XT */

int
_pfm4_start( hwd_context_t * ctx, hwd_control_state_t * ctl )
{
	context_t *pe_ctx = ( context_t * ) ctx;
	control_state_t *pe_ctl = ( control_state_t * ) ctl;
	int ret;

	ret = _pfm4_reset( pe_ctx, pe_ctl );
	if ( ret )
		return ret;
...
int
_pfm4_stop( hwd_context_t * ctx, hwd_control_state_t * ctl )
{
	( void ) ctl;			 /*unused */
	int ret;
	int i;
...
int
_pfm4_ctl( hwd_context_t * ctx, int code, _papi_int_option_t * option )
{
	int ret;
	context_t *pe_ctx = ( context_t * ) ctx;
	...
	case PAPI_MULTIPLEX:
	{
		...
		ret =
			_pfm4_update_control_state( pe_ctl, NULL, pe_ctl->num_events,
										   pe_ctx );
		/*
...
int
_pfm4_shutdown( hwd_context_t * ctx )
{
	context_t *pe_ctx = ( context_t * ) ctx;
	int ret;
...
{
	if ( _pfm4_pmu_type == PFM_PMU_MONTECITO ) {
...
{
	if ( _pfm4_pmu_type == PFM_PMU_ITANIUM2 ) {
...
void
_pfm4_dispatch_timer( int n, hwd_siginfo_t * info, void *uc )
{
	( void ) n;				 /*unused */
	...
	if ( thread == NULL ) {
		PAPIERROR( "thread == NULL in _pfm4_dispatch_timer for fd %d!", fd );
		return;
	}

	if ( thread->running_eventset[cidx] == NULL ) {
		PAPIERROR
			( "thread->running_eventset == NULL in _pfm4_dispatch_timer for fd %d!",
			  fd );
		return;
	}
	...
	if ( thread->running_eventset[cidx]->overflow.flags == 0 ) {
		PAPIERROR
			( "thread->running_eventset->overflow.flags == 0 in _pfm4_dispatch_timer for fd %d!",
			  fd );
		return;
	}
...
int
_pfm4_stop_profiling( ThreadInfo_t * thread, EventSetInfo_t * ESI )
{
	( void ) ESI;			 /*unused */
	int i, ret = PAPI_OK;
	int cidx = MY_VECTOR.cmp_info.CmpIdx;
...
int
_pfm4_set_profile( EventSetInfo_t * ESI, int EventIndex, int threshold )
{
	int ret;
	int evt_idx;
	int cidx = MY_VECTOR.cmp_info.CmpIdx;
	...
	if ( threshold == 0 ) {
		...
		ret = _pfm4_set_overflow( ESI, EventIndex, threshold );
...
int
_pfm4_set_overflow( EventSetInfo_t * ESI, int EventIndex, int threshold )
{
	int cidx = MY_VECTOR.cmp_info.CmpIdx;
	context_t *ctx = ( context_t * ) ( ESI->master->context[cidx] );
	control_state_t *ctl = ( control_state_t * ) ( ESI->ctl_state );
	...
	retval =
		_pfm4_update_control_state( ctl, NULL,
									   ( ( control_state_t * ) ( ESI->
																 ctl_state ) )->
									   num_events, ctx );
...
int
_pfm4_init_control_state( hwd_control_state_t * ctl )
{
...
int
_pfm4_allocate_registers( EventSetInfo_t * ESI )
{
	int i, j;
	for ( i = 0; i < ESI->NativeCount; i++ ) {
		if ( _papi_pfm4_ntv_name_to_bits
			 ( ESI->NativeInfoArray[i].ni_eventname,
...
int
_pfm4_update_control_state( hwd_control_state_t * ctl, NativeInfo_t * native,
...
papi_vector_t _pfm4_vector = {
	...
	.init_control_state = _pfm4_init_control_state,
	.start = _pfm4_start,
	.stop = _pfm4_stop,
	.read = _pfm4_read,
	.shutdown = _pfm4_shutdown,
	.ctl = _pfm4_ctl,
	.update_control_state = _pfm4_update_control_state,
	.update_shlib_info = _pfm4_update_shlib_info,
	.set_domain = set_domain,
	.reset = _pfm4_reset,
	.set_overflow = _pfm4_set_overflow,
	.set_profile = _pfm4_set_profile,
	.stop_profiling = _pfm4_stop_profiling,
	.init_substrate = _pfm4_init_cmp,
	.dispatch_timer = _pfm4_dispatch_timer,
	.get_real_usec = _pfm4_get_real_usec,
	.get_real_cycles = _pfm4_get_real_cycles,
	.get_virt_cycles = _pfm4_get_virt_cycles,
	.get_virt_usec = _pfm4_get_virt_usec,

	/* from OS */
	.get_memory_info = _pfm4_get_memory_info,
	.init = _papi_sub_pe_init,
	.get_dmem_info = _pfm4_get_dmem_info,
	.allocate_registers = _pfm4_allocate_registers,
	.write = _pfm4_write,
//	.ntv_enum_events = _papi_pfm4_ntv_enum_events,
//	.ntv_name_to_code = _papi_pfm4_ntv_name_to_code,
//	.ntv_code_to_name = _papi_pfm4_ntv_code_to_name,
//	.ntv_code_to_descr = _papi_pfm4_ntv_code_to_descr,
//	.ntv_code_to_bits = _papi_pfm4_ntv_code_to_bits,
//	.ntv_bits_to_info = _papi_pfm4_ntv_bits_to_info,
	.ntv_enum_nm_events = _papi_pfm4_ntv_enum_named_events,
	.ntv_name_to_descr = _papi_pfm4_ntv_name_to_descr,
	.ntv_name_to_bits = _papi_pfm4_ntv_name_to_bits,
	.ntv_name_to_info = _papi_pfm4_ntv_name_to_info,
};
